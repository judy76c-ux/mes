...
const SettingsModule = (function() {
        ...
        return `
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="${idPrefix}PartName" placeholder="예: 프론트 범퍼" value="${v('partName')}" oninput="SettingsModule.updateProductInjInfo('${idPrefix}')">
                </div>
                <div class="form-group">
            ...
            <!-- 사출 자재 정보 섹션 -->
            <div style="font-weight:600;color:var(--text-primary);margin:20px 0 8px;padding-bottom:8px;border-bottom:2px solid var(--accent-blue);">
                <span class="material-symbols-outlined" style="vertical-align:middle;font-size:18px;">precision_manufacturing</span>
                사출 자재 정보
            </div>
            <!-- 수동 필터 입력 -->
            <div style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:8px;padding:10px 14px;margin-bottom:8px;">
                <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:8px;">
                    품명 입력 시 자동 매칭 · 아래 필터로 직접 검색 가능 (복수 선택)
                </div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end;">
                    <div style="display:flex;flex-direction:column;gap:3px;min-width:110px;">
                        <label style="font-size:0.75rem;color:var(--text-secondary);font-weight:600;">차종</label>
                        <select id="${idPrefix}InjFiltCar" class="form-select" style="font-size:0.82rem;padding:5px 8px;"
                            onchange="SettingsModule.onProdInjFiltCarChange('${idPrefix}')">
                            <option value="">전체</option>
                            ${[...new Set((Storage.getAll(INJECT_MAT_STORE)||[]).map(m=>m.carModel).filter(Boolean))].sort().map(c=>`<option value="${c}">${c}</option>`).join('')}
                        </select>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:3px;min-width:160px;">
                        <label style="font-size:0.75rem;color:var(--text-secondary);font-weight:600;">사출품명</label>
                        <select id="${idPrefix}InjFiltPart" class="form-select" style="font-size:0.82rem;padding:5px 8px;"
                            onchange="SettingsModule.onProdInjFiltPartChange('${idPrefix}')">
                            <option value="">전체</option>
                        </select>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:3px;min-width:110px;">
                        <label style="font-size:0.75rem;color:var(--text-secondary);font-weight:600;">컬러</label>
                        <select id="${idPrefix}InjFiltColor" class="form-select" style="font-size:0.82rem;padding:5px 8px;"
                            onchange="SettingsModule.onProdInjFiltChange('${idPrefix}')">
                            <option value="">전체</option>
                        </select>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:3px;min-width:80px;">
                        <label style="font-size:0.75rem;color:var(--text-secondary);font-weight:600;">공급처</label>
                        <div id="${idPrefix}InjFiltSupplier" style="padding:5px 8px;background:white;border:1px solid var(--border-color);border-radius:6px;font-size:0.82rem;color:var(--text-muted);min-height:32px;display:flex;align-items:center;">—</div>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:3px;min-width:80px;">
                        <label style="font-size:0.75rem;color:var(--text-secondary);font-weight:600;">단가</label>
                        <div id="${idPrefix}InjFiltPrice" style="padding:5px 8px;background:white;border:1px solid var(--border-color);border-radius:6px;font-size:0.82rem;color:var(--text-muted);min-height:32px;display:flex;align-items:center;">—</div>
                    </div>
                    <button type="button" onclick="SettingsModule.resetProdInjFilter('${idPrefix}')"
                        style="padding:5px 12px;border:1px solid var(--border-color);border-radius:6px;background:transparent;color:var(--text-muted);cursor:pointer;font-size:0.78rem;align-self:flex-end;">
                        초기화
                    </button>
                </div>
            </div>
            <!-- 자동매칭 + 필터 결과 테이블 -->
            <div id="${idPrefix}InjInfo" style="font-size:0.82rem;margin-bottom:12px;">
                <div style="padding:10px 14px;color:var(--text-muted);">품명을 입력하면 매칭된 사출 자재가 표시됩니다.</div>
            </div>
        `;
    }
    ...
    function openAddProductModal() {
        ...
        setTimeout(() => {
            ...
            // 컬러 입력 시 사출 자재 컬러 일치 표시 즉시 갱신
            const colorEl = document.getElementById('addProdColor');
            if (colorEl) colorEl.addEventListener('input', () => updateProductInjInfo('addProd'));
            updateProductInjInfo('addProd');
        }, 100);
    }
    ...
    async function saveProduct() {
        ...
        await Storage.add(PRODUCTS_STORE, data);
        UIUtils.closeModal();
    ...
    function editProduct(id) {
        ...
        `, 'xl');
        setTimeout(() => {
            updateProductInjInfo('editProd');
            const colorEl = document.getElementById('editProdColor');
            if (colorEl) colorEl.addEventListener('input', () => updateProductInjInfo('editProd'));
        }, 100);
    }
    ...
    function _injectMatFormHTML(m = {}) {
        ...
        return `
            ...
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">제작품목1</label>
                    <input type="text" class="form-input" id="imMfgProductName" placeholder="예: 완성 범퍼 (좌)" value="${v('mfgProductName')}">
                </div>
                <div class="form-group">
                    <label class="form-label">제작품목2</label>
                    <input type="text" class="form-input" id="imMfgProductName2" placeholder="예: 완성 범퍼 (우)" value="${v('mfgProductName2')}">
                </div>
            </div>
    ...
    function _collectInjectMatForm() {
        ...
        return {
            ...
            mfgProductName:  g('imMfgProductName').trim(),
            mfgProductName2: g('imMfgProductName2').trim(),
