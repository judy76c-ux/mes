...
const ShippingStandbyModule = (function() {
    ...
    function render(container) {
            ...
            <div class="fade-in-up">
                <div class="page-header">
                    <div class="page-header-left">
                        <h3>출하검사</h3>
                        <p>도장·레이져 검사 완료품의 출하 샘플링 검사(AQL 10)를 관리합니다.</p>
                    </div>
                    <div class="page-actions">
                        ...
                        <button class="btn btn-outline" id="ssToggleAqlBtn"
                            onclick="ShippingStandbyModule.toggleAqlTable()"
                            style="font-size:0.82rem;">
                            <span class="material-symbols-outlined">table_chart</span> AQL 10 표
                        </button>
                    </div>
                </div>

                <!-- AQL 10 참조표 (기본 숨김) -->
                <div id="ssAqlRef" style="display:none;margin-bottom:16px;" class="card">
        ...
        // AQL 표 내용 채우기
        const aqlRef = document.getElementById('ssAqlRef');
        if (aqlRef) aqlRef.querySelector('.card-body').innerHTML =
            `<h4 style="margin:0 0 8px 0;">AQL 10 검사 기준표 (ISO 2859-1, 보통검사 수준Ⅱ)</h4>${AQL10.tableHtml()}`;
    ...
    function loadHistory() {
        ...
        tbody.innerHTML = data.map(d => {
            ...
            const aqlInfo = d.aqlCode ? `${d.aqlCode} (n=${d.sampleSize})` : '-';
            return `
            <tr style="cursor:pointer;" onclick="ShippingStandbyModule._showDetail('${d.id}', event)">
                ...
                <td style="text-align:center;font-size:0.82rem;">
                    ${UIUtils.formatNumber(d.sampleQty || 0)}<br>
                    <span style="font-size:0.68rem;color:var(--text-muted);">${aqlInfo}</span>
                </td>
                ...
                <td style="white-space:nowrap;" onclick="event.stopPropagation()">
                    <button class="btn btn-sm btn-danger"
                        onclick="ShippingStandbyModule.removeHistory('${d.id}')"
                        style="padding:3px 8px;font-size:0.78rem;">삭제</button>
                </td>
            </tr>`;
        }).join('');
    }
    ...
    function _showDetail(id, event) {
        ...
        const aql = d.aqlCode
            ? { code: d.aqlCode, sampleSize: d.sampleSize, ac: d.ac, re: d.re }
            : AQL10.get(d.lotSize || 0);
        ...
        popup.innerHTML = `
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px;text-align:center;">
                ...
                <div style="background:var(--bg-secondary);border-radius:8px;padding:7px 4px;">
                    ...
                    <div style="font-size:0.6rem;color:var(--text-muted);">${aql.code} (Ac≤${aql.ac})</div>
                </div>
    ...
    // ── AQL 표 토글 ──────────────────────────────────────────────────
    function toggleAqlTable() {
        const el = document.getElementById('ssAqlRef');
    ...
    function exportHistory() {
        ...
        const headers = ['검사일', '납품처', '차종', '제품명', '컬러', '도장LOT', '사출LOT',
            'LOT수량', '코드', '샘플수', '불량수', '합격기준', '불합격기준', '판정', '검사자', '비고'];
        const rows = data.map(d => [
            d.date, d.customer||'', d.carModel||'', d.partName||'', d.color||'',
            d.paintingDate||'', d.lotNo||'',
            d.lotSize||0, d.aqlCode||'', d.sampleQty||0, d.defectQty||0, d.ac||0, d.re||0,
...
// ===================================================================
// AQL 10 샘플링 테이블 (ISO 2859-1, 보통 검사, 검사 수준 II)
// ===================================================================
const AQL10 = (function() {
    // [lotMin, lotMax, codeChar, sampleSize, acceptanceNo, rejectionNo]
    const TABLE = [
        [2,     8,     'A', 13,  3,  4],   // ↑ → E 적용
        [9,     15,    'B', 13,  3,  4],   // ↑ → E 적용
        [16,    25,    'C', 13,  3,  4],   // ↑ → E 적용
    ...
    function get(lotSize) {
        const n = parseInt(lotSize) || 0;
        if (n < 2) return { code: '-', sampleSize: n, ac: 0, re: 1 };
        const row = TABLE.find(r => n >= r[0] && n <= r[1]) || TABLE[TABLE.length - 1];
        const sampleSize = Math.min(row[3], n); // 샘플수 > LOT크기 불가
        return { code: row[2], sampleSize, ac: row[4], re: row[5] };
    }
        ...
        <table style="width:100%;border-collapse:collapse;font-size:0.8rem;margin-top:8px;">
            <thead>
                <tr style="background:var(--bg-secondary);">
                    <th style="padding:5px 8px;text-align:left;border:1px solid var(--border);">LOT 크기</th>
                    <th style="padding:5px 8px;text-align:center;border:1px solid var(--border);">코드</th>
                    <th style="padding:5px 8px;text-align:center;border:1px solid var(--border);">샘플수</th>
                    <th style="padding:5px 8px;text-align:center;border:1px solid var(--border);">합격 (Ac)</th>
                    <th style="padding:5px 8px;text-align:center;border:1px solid var(--border);">불합격 (Re)</th>
                </tr>
            </thead>
            <tbody>
                ${TABLE.map(r => `
                <tr>
                    <td style="padding:4px 8px;border:1px solid var(--border);">${UIUtils.formatNumber(r[0])} ~ ${UIUtils.formatNumber(r[1])}</td>
                    <td style="padding:4px 8px;text-align:center;border:1px solid var(--border);font-weight:700;">${r[2]}</td>
                    <td style="padding:4px 8px;text-align:center;border:1px solid var(--border);color:var(--accent-blue);font-weight:600;">${r[3]}</td>
                    <td style="padding:4px 8px;text-align:center;border:1px solid var(--border);color:var(--accent-green);font-weight:600;">${r[4]}</td>
                    <td style="padding:4px 8px;text-align:center;border:1px solid var(--border);color:var(--accent-red);font-weight:600;">${r[5]}</td>
                </tr>`).join('')}
            </tbody>
        </table>`;
    }

    return { get, tableHtml };
})();
...
const ShippingInspectionModule = (function() {
    ...
    // ── AQL 카드 (LOT 크기 기반 자동 계산) ───────────────────────────
    function _aqlCardHtml(lotSize) {
        const aql = AQL10.get(lotSize);
        const pct = lotSize > 0 ? ((aql.sampleSize / lotSize) * 100).toFixed(1) : '-';
        return `
        <div style="background:rgba(59,130,246,0.06);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:12px 16px;">
            <div style="font-size:0.78rem;font-weight:600;color:var(--accent-blue);margin-bottom:10px;">
                AQL 10 샘플링 계획 (ISO 2859-1, 보통검사 수준Ⅱ)
            </div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;text-align:center;">
                <div style="background:var(--bg-primary);border-radius:6px;padding:8px 4px;">
                    <div style="font-size:0.68rem;color:var(--text-muted);">코드 문자</div>
                    <div style="font-weight:700;font-size:1.2rem;color:var(--accent-blue);">${aql.code}</div>
                </div>
                <div style="background:var(--bg-primary);border-radius:6px;padding:8px 4px;">
                    <div style="font-size:0.68rem;color:var(--text-muted);">샘플 수</div>
                    <div style="font-weight:700;font-size:1.2rem;color:var(--accent-blue);">${aql.sampleSize}<span style="font-size:0.7rem;font-weight:400;"> EA</span></div>
                </div>
                <div style="background:var(--bg-primary);border-radius:6px;padding:8px 4px;">
                    <div style="font-size:0.68rem;color:var(--text-muted);">합격 기준 (Ac)</div>
                    <div style="font-weight:700;font-size:1.2rem;color:var(--accent-green);">≤ ${aql.ac}</div>
                </div>
                <div style="background:var(--bg-primary);border-radius:6px;padding:8px 4px;">
                    <div style="font-size:0.68rem;color:var(--text-muted);">불합격 기준 (Re)</div>
                    <div style="font-weight:700;font-size:1.2rem;color:var(--accent-red);">≥ ${aql.re}</div>
                </div>
            </div>
            <div style="margin-top:8px;font-size:0.75rem;color:var(--text-muted);text-align:right;">
                LOT ${UIUtils.formatNumber(lotSize)} EA → 샘플 ${UIUtils.formatNumber(aql.sampleSize)} EA (${pct}%)
            </div>
        </div>`;
    }

    function updateAqlCard() {
        const lotSize = parseInt(document.getElementById('siLotSize')?.value || 0);
        const card = document.getElementById('siAqlCard');
        if (card) card.innerHTML = _aqlCardHtml(lotSize);

        // 샘플수 기본값 업데이트
        const sampleEl = document.getElementById('siSampleQty');
        if (sampleEl && !sampleEl._manuallyEdited) {
            sampleEl.value = AQL10.get(lotSize).sampleSize;
        }
    ...
    function _autoJudge() {
        const lotSize  = parseInt(document.getElementById('siLotSize')?.value  || 0);
        const defects  = parseInt(document.getElementById('siDefectQty')?.value || 0);
        const resultEl = document.getElementById('siResult');
        if (!resultEl) return;
        const aql = AQL10.get(lotSize);
        if (lotSize > 0) {
            resultEl.value = defects <= aql.ac ? '합격' : '불합격';
            resultEl.style.color = resultEl.value === '합격' ? 'var(--accent-green)' : 'var(--accent-red)';
        }
    }
        ...
        <div style="display:grid;gap:14px;">
            ...
            <div class="card"><div class="card-body">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                    ...
                    <div class="form-group" style="margin:0;">
                        <label class="form-label">검사 수량 (LOT 크기) <span style="color:var(--accent-red);">*</span></label>
                        <input type="number" class="form-input" id="siLotSize" value="${sb.inspectionQty || 0}"
                            min="1" oninput="ShippingInspectionModule.updateAqlCard()">
                    </div>
                </div>
            </div></div>

            <!-- AQL 10 자동 계산 -->
            <div id="siAqlCard">${_aqlCardHtml(sb.inspectionQty || 0)}</div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:14px;">
                    <div class="form-group" style="margin:0;">
                        ...
                        <input type="number" class="form-input" id="siSampleQty"
                            value="${AQL10.get(sb.inspectionQty || 0).sampleSize}" min="0"
                    ...
                    <div class="form-group" style="margin:0;">
                        ...
                        <input type="number" class="form-input" id="siDefectQty" value="0" min="0"
                            oninput="ShippingInspectionModule._autoJudge()">
                    </div>
                    <div class="form-group" style="margin:0;">
            ...
            <div class="fade-in-up">
                <div class="page-header">
                    ...
                    <div class="page-actions">
                        ...
                        <button class="btn btn-outline" id="siToggleAqlBtn"
                            onclick="ShippingInspectionModule.toggleAqlTable()"
                            style="font-size:0.82rem;">
                            <span class="material-symbols-outlined">table_chart</span> AQL 10 표
                        </button>
                    </div>
                ...
                <!-- AQL 10 참조 표 (기본 숨김) -->
                <div id="siAqlTableRef" style="display:none;" class="card" style="margin-bottom:16px;">
                    <div class="card-body">
                        <h4 style="margin:0 0 8px 0;">AQL 10 검사 기준표 (ISO 2859-1, 보통검사 수준Ⅱ)</h4>
                        ${AQL10.tableHtml()}
                    </div>
                </div>
    ...
    function toggleAqlTable() {
        const el = document.getElementById('siAqlTableRef');
        if (el) el.style.display = el.style.display === 'none' ? '' : 'none';
    }
        ...
        tbody.innerHTML = data.map(d => {
            ...
            return `
                <tr>
                    ...
                    <td style="text-align:center;font-size:0.85rem;">${UIUtils.formatNumber(d.sampleQty || 0)}<br><span style="font-size:0.7rem;color:var(--text-muted);">${aqlInfo}</span></td>
                    ...
                    <td style="white-space:nowrap;">
                        <button class="btn btn-sm btn-danger" onclick="ShippingInspectionModule.remove('${d.id}')"
                            style="padding:3px 8px;font-size:0.78rem;">삭제</button>
                    </td>
                </tr>
            `;
    ...
    function exportData() {
        ...
        if (!data.length) { UIUtils.toast('데이터가 없습니다.', 'warning'); return; }
        const headers = ['검사일', '납품처', '차종', '제품명', '컬러', '도장LOT', '사출LOT', 'LOT수량', '코드', '샘플수', '불량수', '합격기준', '불합격기준', '판정', '검사자', '비고'];
        const rows = data.map(d => [
            d.date, d.customer||'', d.carModel||'', d.partName||'', d.color||'',
            d.paintingDate||'', d.lotNo||'',
            d.lotSize||0, d.aqlCode||'', d.sampleQty||0, d.defectQty||0, d.ac||0, d.re||0,
...
const ProductWarehouseModule = (function() {
    ...
    function _showHistory(keyEnc, event) {
        ...
        popup.style.cssText = `
            ...
            overflow-y:auto; font-size:0.88rem;
        `;
        popup.style.left = (event.clientX + 14) + 'px';
        popup.style.top  = (event.clientY - 10) + 'px';
        ...
        const rowsHtml = records.length
            ...
            : `<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--text-muted);">이력 없음</td></tr>`;
