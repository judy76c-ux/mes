    ...
    function search() {
        ...
        tbody.innerHTML = data.map(d => `
            <tr>
                ...
                <td>${d.note || '-'}</td>
                <td>
                    <button class="btn btn-sm btn-danger" onclick="PaintingIncomingModule.remove('${d.id}')">삭제</button>
                </td>
            </tr>
        `).join('');
    }
...
const PaintingWorkModule = (function() {
    ...
    function renderWorkList() {
        ...
        tbody.innerHTML = data.map(d => {
            ...
            const actionButtons = isInspectionCompleted ?
                ...
                '<button class="btn btn-sm btn-danger" onclick="PaintingWorkModule.remove(\'' + d.id + '\')">삭제</button>';
    ...
    // ──────────────────────────────────────────────
    // 생산계획 품명(planPartName) → 사출자재의 제작품명1/2 매칭 → injPartName 목록 반환
    function getInjPartNamesForProduct(planPartName, carModel) {
    ...
    function buildInjPartOptionsHtml(planPartName, carModel) {
        var parts = getInjPartNamesForProduct(planPartName, carModel);
    ...
    // injPartName으로 사출 창고 LOT를 엄격하게 조회 (OR 논리 없이 partName 일치만)
    function getInjectionLotsByInjPart(injPartName) {
        ...
        return Object.values(lotMap).filter(function(l) {
                return l.balance > 0;
            })
            .sort(function(a, b) {
                return a.lotNo.localeCompare(b.lotNo); // 선입선출: 오래된 LOT 먼저
            });
    }

    // injPartName 기반 LOT <option> HTML 빌드
    function buildLotOptionsHtmlByInjPart(injPartName) {
        var lots = getInjectionLotsByInjPart(injPartName);
        if (lots.length === 0) return '<option value="" data-balance="">-- 해당 LOT 없음 (직접입력 가능) --</option>';
    ...
    function onInjPartSelect(sel) {
        ...
        if (injPartName) {
            ...
            lotCount = getInjectionLots(cm, pn).length;
            lotsHtml = buildLotOptionsHtml(cm, pn);
        }
    ...
    // 선택 제외 목록을 반영한 LOT 옵션 HTML 생성 (data-balance 포함)
    function _buildFilteredLotOptions(injPartName, carModel, partName, excludeLotNos) {
        var lots = injPartName
            ? getInjectionLotsByInjPart(injPartName)
            : getInjectionLots(carModel || '', partName || '');
    ...
    function _updateLotQtyMax(row, lotNo) {
        ...
        var cm = (document.getElementById('addPwCarModelHidden') || {}).value || '';
        var pn = (document.getElementById('addPwPartNameHidden') || {}).value || '';
        var lots = injPartName ? getInjectionLotsByInjPart(injPartName) : getInjectionLots(cm, pn);
    ...
    function openAddModal(prefill) {
        ...
        // 사출자재 마스터에서 제작품명1/2 매칭 → 사출명 자동 결정
        var injParts = partName ? getInjPartNamesForProduct(partName, carModel) : [];
        ...
        var injPartOptsHtml = buildInjPartOptionsHtml(partName, carModel);
        var lotsHtml = autoInjPartName ?
            buildLotOptionsHtmlByInjPart(autoInjPartName) :
            buildLotOptionsHtml(carModel, partName);
        var initialLotRow = _buildLotRow(lotsHtml, '', '');
        // LOT 추가 버튼 활성화 여부: 사출 창고에 LOT가 2개 이상일 때만 가능
        var initialLotCount = autoInjPartName ?
            getInjectionLotsByInjPart(autoInjPartName).length :
            getInjectionLots(carModel, partName).length;
        ...
        var qtyRowHtml =
            ...
            '<label class="form-label" style="font-size:0.84rem;">투입수량 (IN PUT)' +
            ...
            '<div class="form-group" style="margin:0;">' +
            '<label class="form-label" style="font-size:0.84rem;">산출 수량 (OUT PUT)</label>' +
            ...
            '<div class="form-group" style="margin:0;">' +
            '<label class="form-label" style="font-size:0.84rem;">투입인원 (명)</label>' +
    ...
    async function saveNew() {
        ...
        var allLots = injPartName ? getInjectionLotsByInjPart(injPartName) : getInjectionLots(cm, pn);
        ...
        for (var di = 0; di < data.lots.length; di++) {
            ...
            await Storage.add(INJ_INV_STORE, {
                date: data.date,
                lotNo: dl.lotNo,
                partName: (dlInfo && dlInfo.partName) || data.partName,
