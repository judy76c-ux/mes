...
var InjectionWarehouseModule = (function() {
    ...
    function render(container) {
        container.innerHTML = `
            <div class="fade-in-up">
                <div class="page-header">
                    ...
                    <div class="page-actions">
                        <button class="btn btn-primary" onclick="InjectionWarehouseModule.openAddModal('입고')">
                            <span class="material-symbols-outlined">add_circle</span> 재고 입고
                        </button>
                        <button class="btn btn-danger" onclick="InjectionWarehouseModule.openAddModal('출고')">
                            <span class="material-symbols-outlined">do_not_disturb_on</span> 재고 출고
                        </button>
                ...
                <!-- 입고 대기품 섹션 -->
                <div class="card" style="margin-bottom:20px; border-left:3px solid var(--accent-green);">
                    <div class="card-header" style="display:flex; align-items:center; justify-content:space-between;">
                        <h4 style="display:flex; align-items:center; gap:8px;">
                            <span class="material-symbols-outlined" style="color:var(--accent-green);">move_to_inbox</span>
                            사출 창고 입고 대기품
                            <span style="font-size:0.75rem; color:var(--text-muted); font-weight:400;">(수입 검사 완료품)</span>
                            <span id="injInspStandbyBadge" style="font-size:0.78rem; background:var(--accent-orange,#f59e0b); color:#fff; padding:2px 8px; border-radius:12px; font-weight:600; display:none;"></span>
                        </h4>
                ...
                <!-- 통계 카드 -->
                <div class="stat-cards" id="injInvStats"></div>
    ...
    function loadData() {
        ...
        // ── 통계 카드 ──────────────────────────────────────────────
        const statsEl = document.getElementById('injInvStats');
        if (statsEl) statsEl.innerHTML = `
            ...
            <div class="stat-card green">
                <div class="stat-card-value" style="color:var(--accent-green);">${UIUtils.formatNumber(totalValue)}</div>
                <div class="stat-card-label">총 재공 금액 (₩)</div>
            </div>
            <div class="stat-card purple">
                <div class="stat-card-value">${partCount}</div>
        ...
        const rows = items
            ...
            .map(item => {
                ...
                const r = (typeof ProductionPlanModule !== 'undefined' && ProductionPlanModule._calcInjPlanReserved)
                    ? ProductionPlanModule._calcInjPlanReserved(item.partName, null)
                ...
                if (r.inProgress > 0) {
                    stockHtml = `
                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:1px;">
                            ...
                            <span style="font-size:0.68rem;background:rgba(234,88,12,0.12);color:#ea580c;border:1px solid rgba(234,88,12,0.3);border-radius:3px;padding:0 4px;white-space:nowrap;">사용중 -${UIUtils.formatNumber(r.inProgress)}</span>
                        </div>`;
                } else if (r.pending > 0) {
                    stockHtml = `
                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:1px;">
                            <span style="font-size:0.7rem;color:var(--text-muted);text-decoration:line-through;">${UIUtils.formatNumber(item.stock)}</span>
                            <span style="font-size:0.85rem;font-weight:700;color:${available > 0 ? 'var(--accent-blue)' : 'var(--accent-red)'};">${UIUtils.formatNumber(available)} EA</span>
                            <span style="font-size:0.68rem;background:rgba(234,179,8,0.12);color:#ca8a04;border:1px solid rgba(234,179,8,0.3);border-radius:3px;padding:0 4px;white-space:nowrap;">예약 -${UIUtils.formatNumber(r.pending)}</span>
                        </div>`;
                } else {
        ...
        tbody.innerHTML = data.map(d => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <button class="btn btn-sm btn-danger" onclick="InjectionWarehouseModule.remove('${d.id}')">삭제</button>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    function renderInspStandby() {
        const body  = document.getElementById('injInspStandbyBody');
        const badge = document.getElementById('injInspStandbyBadge');
        if (!body) return;
        ...
        if (badge) {
            if (pendingRows.length > 0) {
                badge.textContent = `대기 ${pendingRows.length}건`;
                badge.style.display = '';
            } else {
    ...
    function openAddModal(type = '입고') {
        ...
        UIUtils.showModal(`<span class="material-symbols-outlined" style="vertical-align:middle;color:${colorClass};">${titleIcon}</span> 재고 ${type} 등록`, `
            <div class="form-row">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select class="form-select" id="addInvColor">
                        <option value="">-- 차종 먼저 선택 --</option>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div style="margin-bottom:16px;">
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
                    ...
                    <button type="button" class="btn btn-sm btn-outline" onclick="InjectionWarehouseModule.addInvLotRow()" style="display:flex; align-items:center; gap:4px;">
                ...
                <div style="background:var(--bg-secondary); border:1px solid var(--border); border-radius:8px; padding:10px 12px; margin-bottom:8px;">
                    <div style="display:grid; grid-template-columns:1fr 90px 34px; gap:8px; align-items:center; font-size:0.78rem; color:var(--text-muted); padding-bottom:6px; border-bottom:1px solid var(--border); margin-bottom:6px;">
            ...
            <div id="addInvStockArea" style="display:none; margin-top:8px; padding:12px; background:var(--bg-secondary); border:1px solid var(--border); border-radius:8px;">
                ...
                <div id="lotStockListContainer" style="margin-top:10px; border-top:1px solid var(--border); padding-top:10px;">
                    <label class="form-label" style="font-size:0.8rem; color:var(--text-secondary); margin-bottom:5px; display:block;">상세 LOT별 재고 ${type === '출고' ? '(클릭 시 자동 입력)' : '(참조용)'}</label>
    ...
    function onModalCarModelChange() {
        ...
        document.getElementById('addInvStockArea').style.display = 'none';
    ...
    function updateLotStockList(carModel, partName) {
        ...
        const allStock = Storage.getAll(STORE) || [];
        const filteredStock = allStock.filter(s => s.carModel === carModel && s.partName === partName);
    ...
    function onLotItemSelect(lot, qty) {
        const lotInput = document.getElementById('addInvLot');
        const qtyInput = document.getElementById('addInvQty');
        if (lotInput) lotInput.value = lot === '무표기' ? '' : lot;
        if (qtyInput) {
            qtyInput.placeholder = `최대 ${qty.toLocaleString()} EA`;
            qtyInput.focus();
        }
        UIUtils.toast(`LOT ${lot} 선택됨 (가용: ${qty}EA)`, 'info');
    }
    ...
    function addInvLotRow() {
        ...
        div.style.cssText = 'display:grid; grid-template-columns:1fr 90px 34px; gap:8px; align-items:center; margin-bottom:6px;';
    ...
    async function saveNew() {
        ...
        const data = {
            ...
            type: document.getElementById('addInvType').value,
            carModel: document.getElementById('addInvCarModel').value,
            partName: document.getElementById('addInvPart').value,
