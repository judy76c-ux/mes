...
const ProductionPlanModule = (function() {
        ...
        container.innerHTML = `
            <div class="fade-in-up">
                ...
                <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                    <!-- 도장-A Grid -->
                    <div class="card grid-card" style="flex: 1; min-width: 45%; margin-bottom: 0;">
    ...
    function updatePaintStockPanel(overrideCarModel, overridePartName, overrideColor) {
        ...
        const totalEl = document.getElementById('paintStockTotal');
        ...
        validRows.forEach(r => { if (r.mainId) allIds.add(r.mainId); if (r.hardId) allIds.add(r.hardId); if (r.thinnerId) allIds.add(r.thinnerId); });
        const grandTotal = [...allIds].reduce((s, id) => s + Math.max(0, _paintMatBalance(id)), 0);

        panel.style.display = 'block';
        if (totalEl) {
            totalEl.textContent = UIUtils.formatNumber(grandTotal) + ' 개';
            totalEl.style.color = grandTotal > 0 ? 'var(--accent-green)' : 'var(--accent-red)';
        }
    ...
    // ── 사출 재고 조회 헬퍼 ──────────────────────────────────────────
    // 생산계획 품명 → 사출자재 마스터(제작품명1/2) 매칭 → injPartName 목록 반환
    function getInjPartNamesForPlan(planPartName, carModel) {
        if (!planPartName) return [];
        const materials = Storage.getAll(DB.STORES.INJECTION_MATERIALS) || [];
        const seen = {};
        return materials.filter(m => {
            const nameMatch = m.mfgProductName === planPartName || m.mfgProductName2 === planPartName;
            const modelMatch = !carModel || m.carModel === carModel;
            return nameMatch && modelMatch && m.injPartName && !seen[m.injPartName] && (seen[m.injPartName] = true);
        });
    }

    // 사출자재 마스터 목록 기준 창고 재고 집계
    // matList: [{injPartName, injColor, carModel}, ...] (getInjPartNamesForPlan 반환값)
    // 재고 item.color 가 해당 자재의 injColor 와 일치하는 것만 포함
    // item.carModel 도 자재의 carModel 과 일치하는 것만 포함
    function getInjStockLots(matList) {
        ...
        matList.forEach(m => {
            ...
            if (m.injColor) {
                m.injColor.split(/[,，\/]/).map(c => c.trim().toLowerCase()).filter(Boolean)
        ...
        all.forEach(item => {
            ...
            // 차종 필터 — 자재에 차종이 지정된 경우에만 적용 (item.carModel이 없으면 허용)
            ...
            // 사출자재 injColor 기준 컬러 필터
            const allowedColors = matColorMap[item.partName];
            if (allowedColors.size > 0) {
                const iColor = (item.color || '').trim().toLowerCase();
                const match = [...allowedColors].some(c => iColor === c || iColor.includes(c) || c.includes(iColor));
                if (!match) return;
            }

            const effectiveLotNo = item.lotNo || '미기재';
            const key = `${item.partName}|${item.color || ''}|${effectiveLotNo}`;
    ...
    function _calcInjPlanReserved(injPartName, excludePlanId) {
        const injMats = Storage.getAll(DB.STORES.INJECTION_MATERIALS) || [];
        // injPartName → 이 자재가 쓰이는 제품명(partName) 역방향 조회
        const planPartNames = new Set();
        injMats.forEach(m => {
            if (m.injPartName !== injPartName) return;
            if (m.mfgProductName)  planPartNames.add(m.mfgProductName);
            if (m.mfgProductName2) planPartNames.add(m.mfgProductName2);
        });
        if (planPartNames.size === 0) return { pending: 0, inProgress: 0 };
        ...
        allPlans.forEach(p => {
            ...
            if (!planPartNames.has(p.partName)) return;
    ...
    function updateInjStockPanel(overridePartName, overrideCarModel) {
        ...
        let matched = getInjPartNamesForPlan(partName, carModel);
        if (matched.length === 0 && carModel) matched = getInjPartNamesForPlan(partName, '');

        const lots  = getInjStockLots(matched);
        ...
        Object.values(grouped).forEach(g => {
            const r = _calcInjPlanReserved(g.partName, currentPlanId);
        ...
        lotsEl.innerHTML = Object.values(grouped).map(g => {
            ...
            const stockStr    = (r.pending > 0 || r.inProgress > 0)
                ? `<span style="color:var(--text-muted);text-decoration:line-through;font-size:0.75rem;">${UIUtils.formatNumber(g.balance)}</span>
    ...
    function editSlot(slot, line) {
        ...
        if (partValue) {
            try {
                let _im = getInjPartNamesForPlan(partValue, modelValue);
                if (_im.length === 0 && modelValue) _im = getInjPartNamesForPlan(partValue, '');
                const _ilots = getInjStockLots(_im);
        ...
        // 도료 재고 사전 계산
        let _paintDisplay = 'none', _paintTotalHtml = '-', _paintLotsHtml = '';
        if (modelValue && partValue) {
            try {
                ...
                if (_vrows.length > 0) {
                    ...
                    const _pids = new Set();
                    _vrows.forEach(r => {
                        if (r.mainId)    _pids.add(r.mainId);
                        if (r.hardId)    _pids.add(r.hardId);
                        if (r.thinnerId) _pids.add(r.thinnerId);
                    });
                    const _ptotal = [..._pids].reduce((s, id) => s + Math.max(0, _paintMatBalance(id)), 0);
                    _paintTotalHtml = `${UIUtils.formatNumber(_ptotal)} 개`;
        ...
        UIUtils.showModal(`[${line}] 생산 계획 등록`, `
            ...
            <div id="paintStockPanel" style="display:${_paintDisplay}; margin-bottom:12px; padding:10px 14px; background:var(--bg-secondary); border-radius:8px; border:1px solid var(--border-color);">
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px;">
                    ...
                    <span id="paintStockTotal" style="font-size:0.9rem; font-weight:700; color:var(--accent-green);">${_paintTotalHtml}</span>
                </div>
        ...
        if (existingId) {
            if (!carModel && !partName && planQty === 0) {
                ...
                await Storage.update(STORE, existingId, {
                    slot: newSlot, carModel, partName, color, itemType, planQty, startTime, endTime, status
                });
            }
        } else {
            if (carModel || partName || planQty > 0) {
                await Storage.add(STORE, {
                    date, line, slot: newSlot, carModel, partName, color, itemType, planQty, startTime, endTime, status
                });
            }
        }
