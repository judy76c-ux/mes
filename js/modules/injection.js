...
var InjectionIncomingModule = (function() {
        ...
        `;
            ...
            <!-- 수입검사 기준 사진 미리보기 -->
            <div id="injStdPhotoPreview" style="display:none;margin:-8px 0 14px;
                background:var(--bg-secondary);border:1px solid var(--border-color);
                border-radius:8px;padding:10px 12px;">
                <div style="font-size:0.78rem;font-weight:600;color:var(--accent-blue);
        ...
        `);
            ...
            function _refreshStdPreview(carModel, partName) {
                ...
                const stds = (Storage.getAll(DB.STORES.INJ_INSP_STANDARDS) || [])
                    .filter(s => s.carModel === carModel && s.partName === partName);

                if (!stds.length) { wrap.style.display = 'none'; return; }

                const TYPE_LABEL = { product:'제품', box:'박스/용기', other:'기타' };
                grid.innerHTML = stds.map(s => `
                    <div style="text-align:center;">
                        <div style="font-size:0.72rem;color:var(--text-muted);margin-bottom:3px;font-weight:600;">
                            ${TYPE_LABEL[s.photoType] || s.photoType}</div>
                        <img src="${s.imageData}" alt="${s.photoType}"
                             style="width:120px;height:90px;object-fit:cover;border-radius:6px;
                                    border:2px solid var(--border-color);cursor:pointer;"
                             onclick="InjectionIncomingModule._enlargePhoto(this.src, '${s.photoType}')">
                        ${s.note ? `<div style="font-size:0.7rem;color:var(--text-muted);margin-top:2px;
                                               max-width:120px;overflow:hidden;text-overflow:ellipsis;
                                               white-space:nowrap;" title="${s.note}">${s.note}</div>` : ''}
                    </div>`).join('');
            ...
            function _enlargePhoto(src, label) {
                const TYPE_LABEL = { product:'제품', box:'박스/용기', other:'기타' };
                const old = document.getElementById('injPhotoEnlarge');
                if (old) old.remove();
                const div = document.createElement('div');
                div.id = 'injPhotoEnlarge';
                div.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:99999;' +
                    'display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;';
                div.innerHTML = `
                    <div style="color:#fff;font-size:0.85rem;margin-bottom:10px;">
                        ${TYPE_LABEL[label]||label} — 클릭하면 닫힙니다</div>
        ...
        overlay.innerHTML = `
            <div style="max-width:1000px;margin:0 auto;width:100%;background:var(--bg-primary);
                ...
                <div style="background:var(--bg-tertiary);padding:12px 16px;
                    ...
                    <span style="font-weight:700;font-size:1rem;">
                        <span class="material-symbols-outlined"
                              style="vertical-align:middle;font-size:18px;margin-right:4px;">photo_library</span>
                        수입검사 기준 사진 관리
                    </span>
                ...
                <div id="stdAddFormWrap" style="display:none;padding:14px 16px;
                    ...
                    <div style="font-weight:600;margin-bottom:10px;color:var(--accent-blue);font-size:0.9rem;">
                        새 기준 사진 등록
                    </div>
                    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:10px;">
                        ...
                        <div class="form-group" style="margin:0;">
                            <label class="form-label" style="font-size:0.78rem;">메모</label>
                            <input type="text" class="form-input" id="stdNewNote" placeholder="간단 메모">
                        </div>
                    </div>
                    <div style="display:flex;gap:12px;align-items:flex-end;">
                        <div class="form-group" style="margin:0;flex:1;">
                            <label class="form-label" style="font-size:0.78rem;">사진 파일 *</label>
                            <input type="file" class="form-input" id="stdNewFile"
                                accept="image/*" style="padding:4px;"
                                onchange="InjectionIncomingModule._stdPreviewNew(this)">
                        </div>
                        <div id="stdNewThumb" style="width:80px;height:60px;border-radius:6px;
                            border:1px dashed var(--border-color);display:flex;align-items:center;
                            justify-content:center;overflow:hidden;background:var(--bg-secondary);
                            flex-shrink:0;">
                ...
                <!-- 목록 -->
                <div id="stdListWrap" style="padding:14px 16px;min-height:200px;"></div>
            </div>`;
    ...
    function _renderStdList() {
        ...
        const items = (Storage.getAll(STD_STORE) || [])
            ...
            .sort((a,b) => (a.carModel+a.partName+a.photoType)
                         .localeCompare(b.carModel+b.partName+b.photoType, 'ko'));

        if (!items.length) {
            wrap.innerHTML = `<div style="text-align:center;padding:40px;color:var(--text-muted);">
                <span class="material-symbols-outlined" style="font-size:40px;display:block;margin-bottom:8px;">
                    photo_library</span>
                등록된 기준 사진이 없습니다.<br>
                <small>상단 "사진 등록" 버튼으로 추가하세요.</small></div>`;
            return;
        }
        ...
        wrap.innerHTML = Object.values(groups).map(g => `
            <div style="margin-bottom:20px;border:1px solid var(--border-color);
                ...
                <div style="background:var(--bg-tertiary);padding:8px 12px;font-weight:600;
                            font-size:0.88rem;border-bottom:1px solid var(--border-color);">
                    <span class="badge badge-blue" style="margin-right:6px;">${g.carModel}</span>
                    ${g.partName}
                    <span style="color:var(--text-muted);font-weight:400;font-size:0.78rem;
                                 margin-left:6px;">(${g.photos.length}장)</span>
                </div>
                <div style="display:flex;flex-wrap:wrap;gap:12px;padding:12px;">
                    ...
                    <div style="position:relative;text-align:center;">
                        <div style="font-size:0.72rem;font-weight:600;color:var(--text-muted);
                                    margin-bottom:4px;">${PHOTO_TYPE_LABELS[s.photoType]||s.photoType}</div>
                        <img src="${s.imageData}" alt="${s.photoType}"
                             style="width:140px;height:105px;object-fit:cover;border-radius:6px;
                                    border:2px solid var(--border-color);cursor:pointer;"
                             onclick="InjectionIncomingModule._enlargePhoto(this.src,'${s.photoType}')">
                        ${s.note ? `<div style="font-size:0.7rem;color:var(--text-muted);margin-top:3px;
                                               max-width:140px;overflow:hidden;text-overflow:ellipsis;
                                               white-space:nowrap;">${s.note}</div>` : ''}
                        <button class="btn btn-sm btn-danger"
                            style="position:absolute;top:20px;right:2px;padding:1px 5px;font-size:0.7rem;
                                   opacity:.8;"
