...
const PaintingWorkModule = (function() {
    ...
    function buildLaserWipLotOptionsHtml(carModel, partName) {
        ...
        if (lots.length === 0) return '<option value="" data-balance="">-- 재공품 재고 없음 --</option>';
    ...
    function _siteRemainLotsFor(data) {
        try {
            if (typeof PaintingInputModule === 'undefined' || !PaintingInputModule.getLotsByCarPart) return [];
            var injPart = String(data.injPartName || data.partName || '').trim();
            if (!injPart) return [];
            var lots = PaintingInputModule.getLotsByCarPart(data.line, data.carModel, injPart) || [];
            // 이번 실적에서 차감될 수량을 미리 반영해 "저장 후 잔량"을 만든다
            var useByLot = {};
            (data.lots || []).forEach(function (l) {
                var n = String((l && l.lotNo) || '').trim();
                if (n) useByLot[n] = (useByLot[n] || 0) + (Number(l.qty) || 0);
            });
            return lots.map(function (l) {
                return Object.assign({}, l, { balance: (Number(l.balance) || 0) - (useByLot[l.lotNo] || 0) });
    ...
    function _showSiteRemainModal() {
        ...
        UIUtils.showModal('현장에 남는 자재 처리',
            ...
            '이 실적을 저장하면 현장에 <strong style="color:#b45309;">' + UIUtils.formatNumber(ctx.remainQty) + ' EA</strong>가 남습니다. ' +
    ...
    function _renderSiteReturnRows() {
        ...
        lotContainer.querySelectorAll('.pw-lot-row').forEach(function (row) {
            ...
            if (!lotNo) return;
            seenLotNos[lotNo] = true;
            var bal = opt ? Number(opt.getAttribute('data-balance')) : NaN;
            if (isNaN(bal) || bal <= 0) return;
            var qty = Number(qtyInput.value) || 0;
            var leftover = bal - qty;
            if (leftover <= 0.001) return;
            var color = opt ? (opt.getAttribute('data-color') || '') : '';
            candidates.push({ lotNo: lotNo, color: color, leftover: leftover });
        });

        // (b) ②에 행으로 추가하지 않은, 같은 도장작업일 현장입고분 전량
        ...
        if (workDate) {
            var maps = _computeSiteInboundLotMaps({ line: _currentLine, carModel: carModel, date: workDate, partName: injPartName });
            if (maps.ok) {
                Object.keys(maps.receivedQtyByLot).forEach(function (lotNo) {
                    if (seenLotNos[lotNo]) return;
                    var diff = (maps.receivedQtyByLot[lotNo] || 0) - (maps.usedQtyByLot[lotNo] || 0);
                    if (diff > 0.001) candidates.push({ lotNo: lotNo, color: '', leftover: diff });
                });
            }
        }
    ...
    async function saveNew() {
        ...
        // 현장 잔량 정산 확인 — 실적만 등록하고 남은 자재를 방치하면 사출창고 재고와
        // 실물이 계속 어긋나고, 그 잔량이 나중에 통째로 "유실"로 잡힌다.
        // 잔량이 있으면 '이월'인지 '반납'인지 명시하게 하고 그 선택을 실적에 남긴다.
        if (!_siteRemainDeclared()) {
    ...
    function _siteMaterialBalance(d) {
        ...
        try {
            ...
            (Storage.getAll(DB.STORES.PAINTING_INPUT_INVENTORY) || []).forEach(function (r) {
                if (!r || String(r.date || '').slice(0, 10) !== day) return;
                ...
                if (injPartName && String(r.partName || '').trim() && String(r.partName || '').trim() !== injPartName) return;
                var qty = Number(r.quantity) || 0;
                if (qty <= 0) return;
                res.hasLedger = true;
                if (String(r.type || '') === '입고') res.inQty += qty;
                else if (/반납/.test(String(r.source || ''))) res.returnQty += qty;
                else res.usedQty += qty;
            });
    ...
    function _buildWorkAlertsInner(d) {
        ...
        if (actualInput > 0 && shortfall > Math.max(1, actualInput * 0.01)) {
            // 남은 소재 추정치: 계획을 알면 "입고 − 계획"(생산은 계획만큼만 썼다고 가정),
            // 계획을 모르면 현장 원장 잔량으로 대체
            var estRemain = planQty > 0 ? Math.max(0, actualInput - planQty) : Math.max(0, site.remain);
            // 이 실적이 자기 몫(같은 planId로 분출된 자재)을 다 썼다면 반납 버튼이 안 뜬다 —
            ...
            alerts.push(
                ...
                (planQty > 0
                    ? '생산 계획은 <strong>' + UIUtils.formatNumber(planQty) + ' EA</strong>인데 현장 투입자재는 <strong>' + UIUtils.formatNumber(actualInput) + ' EA</strong>임.<br>'
                    : '작업일지 투입수량 합계 <strong>' + UIUtils.formatNumber(loggedInput) + ' EA</strong> · 현장 투입자재 <strong>' + UIUtils.formatNumber(actualInput) + ' EA</strong>임.<br>') +
                (site.laserWip
                    ? '이 제품은 레이져 후 도장 제품이라 레이저 재공품에서 자동 관리됩니다. <strong>반납 조작이 필요 없습니다.</strong>'
                    : '남은 소재는 <strong style="color:#b45309;">' + UIUtils.formatNumber(estRemain) + ' EA</strong>로 추정됨.<br>' +
                      (rb.returnable > 0
                ...
                (rb.returnable > 0
                    ? '<div style="margin-top:9px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">' +
                        ...
                        UIUtils.formatNumber(rb.returnable) + ' EA</strong> (분출 ' + UIUtils.formatNumber(rb.issued) +
    ...
    function _dayInboundLotDetailMap(maps) {
        ...
        (Storage.getAll(DB.STORES.PAINTING_INPUT_INVENTORY) || []).forEach(function (r) {
            if (String(r.type || '') !== '입고') return;
            if (String(r.line || r.paintLine || '').replace(/\s/g, '') !== maps.line) return;
            if (String(r.carModel || '') !== maps.carModel) return;
            if (maps.injPartName && String(r.partName || '') !== maps.injPartName) return;
            if (_resolveInboundStamp(r).slice(0, 10) !== maps.day) return;
            var stamp = _resolveInboundStamp(r);
            var time = stamp.length > 11 ? stamp.slice(11, 16) : '-';
            var lots = (Array.isArray(r.lots) && r.lots.length) ? r.lots : (r.lotNo ? [{ lotNo: r.lotNo }] : []);
            lots.forEach(function (l) {
    ...
    function addMissingLotRow(btnEl) {
        ...
        if (!container) return;
        var excludeLots = _getSelectedLotNos(null);
        // 이 LOT이 이미 다른 행에 선택돼 있으면 같은 미반영분을 다시 새 행으로 추가하지 않는다.
        // (경고 박스는 저장된 실적 기준으로만 "미반영"을 계산해, 방금 UI에서 추가한 행은 모르는
        // 채로 남아 있어 같은 버튼을 두 번 누르면 동일 LOT·동일 잔량이 중복 행으로 쌓였다.)
        if (excludeLots.indexOf(lotNo) >= 0) {
            var dupRow = null;
            container.querySelectorAll('.pw-lot-row').forEach(function (row) {
                if (dupRow) return;
                var s = row.querySelector('.pw-lot-sel');
                if (s && s.value === lotNo) dupRow = row;
            });
        ...
        if (!lotOptRe.test(lotsHtml)) {
            var missingOpt = '<option value="' + _pwEsc(lotNo) + '" data-balance="' + qty + '" selected>' +
                '이 도장작업일 미반영분 │ LOT ' + _pwEsc(lotNo) + ' │ ' + UIUtils.formatNumber(qty) + ' EA</option>';
