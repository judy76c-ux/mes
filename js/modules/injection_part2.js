...
var InjectionWarehouseModule = (function() {
    ...
    // 수입검사에서 연 입고 폼 — LOT·수량을 합격수량으로 고정 (입력·수정 불가)
    let _lockedInspInbound = null;

    function _isInspectionQtyLockedRecord(d) {
        if (!d || d.type !== '입고') return false;
        if (_isSiteReturnInbound(d)) return false;
        if (d.inspId || d.inspLinkedManually) return true;
        return /수입검사/.test(String(d.source || ''));
    }
    ...
    let _siteInboundShortageAcks = {}; // { [car||part||color]: { ackedQty, reason, reasonCode, resolvedAt, resolvedBy, ... } }
    let _siteInboundShortageAcksLoaded = false;

    // 장기 데이터 분석용 기본 사유 (코드는 고정, 화면 라벨만 변경 가능)
    const SITE_INBOUND_SHORTAGE_REASON_OPTIONS = [
        { code: 'material_inbound_delay', label: '자재 입고 지연' },
        { code: 'stock_qty_mismatch', label: '현재 창고 재고 수량 불일치' },
        { code: 'material_shortage_blocked', label: '자재부족 입고 불가' },
        { code: 'material_defect', label: '자재 불량' }
    ];

    function _findSiteInboundShortageReason(code) {
        const key = String(code || '').trim();
        for (let i = 0; i < SITE_INBOUND_SHORTAGE_REASON_OPTIONS.length; i++) {
            if (SITE_INBOUND_SHORTAGE_REASON_OPTIONS[i].code === key) {
                return SITE_INBOUND_SHORTAGE_REASON_OPTIONS[i];
            }
        }
        return null;
    }

    function _composeSiteInboundShortageReason(categoryLabel, extraNote) {
    ...
    function _readSiteInboundShortageReasonForm() {
        const sel = document.getElementById('injSiteInboundShortageReasonCode');
        const noteEl = document.getElementById('injSiteInboundShortageReason');
        const opt = _findSiteInboundShortageReason(sel ? sel.value : '');
        const extra = noteEl ? String(noteEl.value || '').trim() : '';
        return {
            code: opt ? opt.code : '',
            category: opt ? opt.label : '',
            extra: extra,
            reason: _composeSiteInboundShortageReason(opt ? opt.label : '', extra)
        };
    }
    ...
    /** 도장현장 반납이 리워크 재공으로 돌아가야 하는지 — IL 품명뿐 아니라 리워크 입고 반납 플래그 */
    function _isReworkSiteReturn(r) {
        if (!r) return false;
        if (r.isReworkReturn || r.refReworkOutId) return true;
        ...
        function _show(savedIds) {
            UIUtils.showModal(
                ...
                '<div class="form-group">' +
                    ...
                    '<div style="font-size:0.75rem;color:var(--text-muted);margin:-2px 0 8px;">' +
                        '기본 항목은 장기 데이터 분석용으로 반드시 선택하세요.</div>' +
                    '<select id="injSiteInboundShortageReasonCode" class="form-select" style="width:100%;">' +
                        '<option value="">선택 필요</option>' +
                        SITE_INBOUND_SHORTAGE_REASON_OPTIONS.map(function (o) {
                            return '<option value="' + o.code + '">' + _escapeHtml(o.label) + '</option>';
                        }).join('') +
                    '</select>' +
                '</div>' +
                '<div class="form-group">' +
                    '<label class="form-label">입력 추가 사유 <span style="color:var(--accent-red);">*</span></label>' +
                    '<textarea id="injSiteInboundShortageReason" class="form-input" rows="2" ' +
                        'placeholder="' + _escapeHtml(extraPlaceholder) + '" style="width:100%;resize:vertical;"></textarea>' +
                '</div>' +
    ...
    async function confirmSiteInboundShortageResolve(carEnc, partEnc, colorEnc, need, stock, shortage, fromReworkFlag) {
        const formReason = _readSiteInboundShortageReasonForm();
        if (!formReason.code) {
            UIUtils.toast('사유 기본 항목을 선택하세요.', 'warning');
            return;
        }
        if (!formReason.extra) {
            UIUtils.toast('입력 추가 사유를 입력하세요.', 'warning');
            return;
        }
        const reason = formReason.reason;
        ...
        const reportBody =
            ...
            '사유 구분: ' + formReason.category + '\n' +
            '추가 사유: ' + formReason.extra + '\n' +
            '보고자: ' + (actorLabel || '-');
        ...
        _siteInboundShortageAcks[key] = {
            ...
            reason: reason,
            reasonCode: formReason.code,
            reasonCategory: formReason.category,
            reasonNote: formReason.extra,
    ...
    /** 리워크 소재 부족 행을 "이미 리워크 자재를 사용해 도장 완료" 사유로 즉시 취소 처리한다.
     *  현장 실적 입력 화면의 [리워크에서 투입 처리] 버튼을 거치지 않고(이미 보유 중이던 리워크
     *  LOT을 그냥 ②투입 목록에서 골라 썼거나, 시스템 반영 전에 실물만 먼저 사용한 경우 등)
     *  현장에서 실제로는 이미 자재를 확보·사용했는데 목록엔 부족으로 계속 남는 경우를 위한
     *  수동 확인 버튼 — 사유 입력·관리자 쪽지 없이 즉시 처리된다(ackReworkUsageFromWork와 동일한
     *  acked 저장소를 공유하므로 그만큼 need 계산에서 바로 빠진다). */
    async function cancelReworkShortage(carEnc, partEnc, colorEnc, shortage) {
        const carModel = decodeURIComponent(carEnc || '');
        const partName = decodeURIComponent(partEnc || '');
        const color = decodeURIComponent(colorEnc || '');
        ...
        const r = await ackReworkUsageFromWork(carModel, partName, color, qty, {
            note: '리워크 자재 사용 확인 — 목록에서 수동 취소 처리'
        });
        if (!r || !r.ok) {
            UIUtils.toast('취소 처리에 실패했습니다.', 'error');
            return;
        }
        UIUtils.toast('리워크 자재 사용으로 취소 처리했습니다.', 'success');
        loadData();
    }
    ...
    async function ackReworkUsageFromWork(carModel, partName, color, qty, opts) {
        opts = opts || {};
        ...
        try {
            ...
            const prev = _siteInboundShortageAcks[key] || {};
            _siteInboundShortageAcks[key] = {
                ackedQty: Math.max(0, Number(prev.ackedQty) || 0) + q,
                reason: reason,
                fromRework: true,
                resolvedAt: (UIUtils.now ? UIUtils.now() : new Date().toISOString().slice(0, 16).replace('T', ' ')),
                resolvedBy: actorLabel,
                history: (Array.isArray(prev.history) ? prev.history : []).concat([{
                    ...
                    at: UIUtils.now ? UIUtils.now() : new Date().toISOString(),
                    by: actorLabel,
                    source: 'painting_work_input'
                }]).slice(-20)
            };
            await Storage.setConfigValue(SITE_INBOUND_SHORTAGE_ACK_KEY, _siteInboundShortageAcks);
        } catch (e) {
            console.warn('[InjectionWarehouse] 리워크 사용 반영 실패:', e);
    ...
    async function _ensurePendingCutoverLoaded(forceReload) {
        ...
        let v = '';
        try {
            v = await Storage.getConfigValue(PENDING_CUTOVER_KEY);
        } catch (e) {
            // 서버 config 조회 자체가 실패한 경우(네트워크·서버 재시작 등) — 실패를 "설정 없음"
            // 으로 오인해 아래에서 오늘 날짜로 확정 저장하면 안 된다. 필터링 없이 이번만 넘어가고,
            // _pendingCutoverLoaded를 true로 고정하지 않아 다음 조회 때 다시 시도한다.
            console.warn('[InjectionWarehouseModule] 입고 대기 컷오버 조회 실패 — 이번 조회는 필터링 없이 표시합니다.', e);
            _pendingCutover = '';
            return _pendingCutover;
        }
        // ⚠ 예전에는 v가 비어 있으면(설정이 없다고 보고) 오늘 날짜를 컷오버로 자동 확정해
        // 서버에 저장했다. 그런데 Storage.getConfig()는 네트워크 오류가 나도 조용히 {}를
        // 반환하므로(js/storage.js), 서버가 잠깐 응답을 못 하거나 재시작 중이었을 뿐인데도
        // "한 번도 설정된 적 없음"으로 오인해 컷오버를 오늘로 덮어써 버렸다 — 그 순간 몇 주치
        // 입고 대기 backlog(예: 80건, 최장 69일 지연)가 화면에서 통째로 사라지는 사고가 실제로
        // 있었다(검사·재고 원본은 그대로라 데이터 손실은 아니지만, 아무도 못 보게 숨겨졌다).
        // 컷오버는 관리자가 의도적으로 설정한 값만 신뢰하고, 비어 있으면 "필터링 없음"으로 둔다.
    ...
    /** 현재고 확정(재고 기준점) — 이 시점 이후 입출고만 재고·LOT 계산에 사용 */
    function _isStockBaselineRecord(d) {
        return !!(d && (d.isStockBaseline || d.baselineAction === 'stock_baseline' || /현재고 확정/.test(String(d.source || ''))));
    }

    /** 재고실사 보정 — 실사수량이 장부수량보다 많아 생긴 차이만큼 넣은 입고.
     ...
     *  수입검사 합격수량과 비교하는 "초과 입고"가 아니다.
     *  예: 검사 1000 → 현장 출고 500 → 사용 300·반납 200 이면 창고는 700 (1000−500+200). */
    function _isSiteReturnInbound(d) {
        if (!d || d.type === '출고') return false;
        if (d.refReturnId || d.isSiteReturn) return true;
        const src = String(d.source || '').trim();
        return src === '도장현장 반납' || src === '도장현장 반납 입고' || src === '현장 반납';
    }

    function _findLatestStockBaseline(records) {
        const list = (records || []).filter(_isStockBaselineRecord);
        if (!list.length) return null;
        return list.slice().sort(function (a, b) {
            return InvCalc.recordStamp(a).localeCompare(InvCalc.recordStamp(b))
    ...
    /**
     * 재고·LOT 계산용 원장.
     * 최신 「현재고 확정」이 있으면 그 기록 + 확정 시각 이후 이동만 재생한다.
     * 입고/출고 이력 탭은 전체 원장을 그대로 쓰므로 이 함수를 쓰지 않는다.
     */
    function _recordsForStockCalc(records) {
        const all = records || [];
        const baseline = _findLatestStockBaseline(all);
        if (!baseline) return all.slice();

        const cut = InvCalc.recordStamp(baseline);
        const cutCreated = String(baseline.createdAt || '');
        const cutId = String(baseline.id || '');
        const after = all.filter(function (r) {
            if (!r || r === baseline) return false;
            if (_isStockBaselineRecord(r)) return false;
            const s = InvCalc.recordStamp(r);
            if (s > cut) return true;
            if (s < cut) return false;
            const c = String(r.createdAt || '');
            if (cutCreated && c) {
                if (c > cutCreated) return true;
                if (c < cutCreated) return false;
            }
            if (cutId && r.id) return String(r.id) > cutId;
            return false;
        });
        return [baseline].concat(after);
    }
    ...
    /** LOT 번호 없이("무표기") 입고되는 건을 막는다. 같은 차종·품명·컬러의 다른 무표기 건과
     *  하나의 재고 버킷(InvCalc)으로 묶여 FIFO 잔량 계산이 서로 뒤섞이는 사고 — 도장현장
     *  쪽에서 실제로 겪었다(방금 도착한 자재인데 무관한 과거 무표기 출고에 흡수돼 "잔량 0"으로
     *  보임) — 를 창고 입고 단계에서 원천적으로 막는다. 출고(type:'출고')는 손대지 않는다:
     *  출고는 항상 기존 LOT을 지정해서 나가야 하므로 새 LOT을 만들어 붙이면 오히려
     *  차감 대상을 못 찾게 만든다. */
    function _ensureUniqueLotOnInbound(record) {
        if (!record || record.type !== '입고') return record;
        const day = String(record.date || (UIUtils.today ? UIUtils.today() : '') || '').slice(0, 10).replace(/-/g, '').slice(2) || '000000';
        let seq = 0;
        function freshLot() {
            seq++;
            return 'DT' + day + Date.now().toString().slice(-5) + (seq > 1 ? '-' + seq : '');
        }
        if (Array.isArray(record.lots) && record.lots.length) {
            record.lots = record.lots.map(function (l) {
                let lotNo = String((l && l.lotNo) || '').trim();
                if (!lotNo || lotNo === '무표기') lotNo = freshLot();
                return { lotNo: lotNo, qty: Number(l && l.qty) || 0 };
            });
            record.lotNo = record.lots[0].lotNo;
        } else {
            let lotNo = String(record.lotNo || '').trim();
            if (!lotNo || lotNo === '무표기') {
                lotNo = freshLot();
                record.lotNo = lotNo;
                record.lots = [{ lotNo: lotNo, qty: Number(record.quantity) || 0 }];
            }
        }
        return record;
    }
    ...
    function _renderInvHistoryRow(step, isLast, showActions) {
        ...
        const isBaseline = _isStockBaselineRecord(d);
        ...
        const typeBadge = isUnmatchedAct
            ...
            : (isBaseline
                ? `<span style="font-size:0.72rem;font-weight:700;padding:1px 7px;border-radius:999px;
                    background:rgba(15,118,110,.12);color:#0f766e;">기준</span>
                   <span style="margin-left:4px;font-size:0.65rem;font-weight:700;background:#0f766e;color:#fff;padding:1px 6px;border-radius:10px;">현재고 확정</span>`
                : `<span style="font-size:0.72rem;font-weight:700;padding:1px 7px;border-radius:999px;
        ...
        const rowBg = isLast
            ...
            : (isUnmatchedAct ? ' style="background:rgba(180,83,9,.06);"'
                : (isBaseline ? ' style="background:rgba(15,118,110,.06);"'
        ...
        const stampLabel = isUnmatchedAct ? '창고 보정' : (isBaseline ? '현재고 확정' : (isOut ? '창고 출고' : '창고 입고'));
        ...
        return `
            <tr${rowBg}>
                ...
                <td style="text-align:right;font-weight:600;color:${isUnmatchedAct ? '#b45309' : (isBaseline ? '#0f766e' : (isOut ? 'var(--accent-red)' : 'var(--accent-green)'))};">
                    ${qtyHtml}
                </td>
    ...
    function render(container) {
        const actionCards = `
            <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-left:auto;">
                ...
                ${_canEditWarehouseLot() ? ProdAppleMenu.card({ label: '재고실사', subtitle: '실사수량 일괄 보정', icon: 'fact_check', accent: '#0f766e', onClick: "InjectionWarehouseModule.openStockCountModal()" }) : ''}
            </div>`;
    ...
    function _buildDisplayStockMap() {
        ...
        Object.keys(stockMap).forEach(function(key) {
            ...
            const bal = InvCalc.lotBalances(_recordsForStockCalc(_filterProductRecords(g.carModel, g.partName, g.color)));
            g.stock = bal.total;
    ...
    function renderCarTiles(stockMapArg, dataArg) {
        ...
        // 양산품(주로 사용) 차종 먼저, A/S품 전용 차종은 뒤로 — 같은 그룹 안에서는 품목 수 내림차순
        entries.sort(([carA, a], [carB, b]) => {
            const rankA = _carModelSortRank(carA);
            const rankB = _carModelSortRank(carB);
            return (rankA - rankB) || (b.length - a.length) || a[0].carModel.localeCompare(b[0].carModel);
        });
    ...
    function filterTransactions(tab) {
        ...
        const filtered = data.filter(d => {
            ...
            // 현재고 확정은 실물 입고가 아님 — 입고이력 탭에 넣지 않음(기존 type:'입고' 확정 건 포함)
            if (typeFixed === '입고' && _isStockBaselineRecord(d)) return false;
            return true;
        });
    ...
    //   입고 → 수입검사 / 현장 반납 / 수동입고,  출고 → 생산 차감 / 수동 차감
    ...
    function _invRoute(d) {
        ...
        if (d && _isStockBaselineRecord(d)) {
            const reason = String(d.baselineReason || d.resetReason || d.note || '').trim();
            return {
                label: '현재고 확정',
                color: '#0f766e',
                detail: reason || '재고 계산 기준점 · 이전 이력은 조회만'
            };
        }
        if (d && _isSiteReturnInbound(d)) {
            return {
                label: '현장 반납',
                color: '#059669',
                detail: src || '도장현장 반납 · 출고분의 재입고'
            };
        }
    ...
    function _routeLinkFor(d, route) {
        ...
        if (route.label === '수동입고' || route.label === '직접 입고' || route.label === '현장 반납') {
            ...
            return {
                ...
                title: route.label === '현장 반납' ? '입고 이력에서 이 반납 건 보기' : '입고 이력에서 이 품목 보기'
            };
        }
        return null;
    }
    ...
    function showPartDetail(carModel, partName, color) {
        ...
        const stockItems = _recordsForStockCalc(items);
        const baselineRec = _findLatestStockBaseline(items);

        const mat   = mats.find(m => m.carModel === carModel && m.injPartName === partName);
        const price = Number(mat ? mat.unitPrice : 0) || 0;

        const { balance, lots: lotList } = StockDetailUI.lotBalancesFromRecords(stockItems, { positiveOnly: false });
        ...
        const baselineStamp = baselineRec ? InvCalc.recordStamp(baselineRec) : '';
        const baselineBanner = baselineRec ? `
            <div style="margin-bottom:14px;padding:10px 12px;border-radius:8px;border:1px solid rgba(15,118,110,.3);
                        background:rgba(15,118,110,.07);font-size:0.8rem;line-height:1.5;color:var(--text-secondary);">
                <strong style="color:#0f766e;">재고 계산 기준</strong>
                · ${ _escapeHtml(baselineStamp || String(baselineRec.date || '').slice(0, 16)) }
                확정 이후 입출고만 현재 재고·LOT에 반영됩니다. 아래 입출고 이력·입고/출고 이력 탭의 과거 기록은 그대로 조회됩니다.
            </div>` : `
            <div style="margin-bottom:14px;padding:10px 12px;border-radius:8px;border:1px solid rgba(245,158,11,.35);
                        background:rgba(245,158,11,.07);font-size:0.8rem;line-height:1.5;color:var(--text-secondary);">
                아직 <strong>현재고 확정</strong>이 없습니다. 과거 전체 이력으로 재고를 계산 중입니다.
                ${_isAdminUser() ? '관리자는 상단 <strong>현재고 확정</strong>으로 기준점을 만들 수 있습니다.' : ''}
            </div>`;
        ...
        UIUtils.showModal(
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;padding:10px 14px;
                ...
                <div style="margin-left:auto;display:flex;gap:6px;flex-shrink:0;flex-wrap:wrap;justify-content:flex-end;">
                    ...
                    ${_isAdminUser() ? `
                    <button class="btn btn-sm btn-outline" style="font-size:0.78rem;color:#0f766e;border-color:#0f766e;"
                        title="현재 잔량 LOT을 재고 계산 기준점으로 확정합니다. 과거 이력은 삭제되지 않습니다."
                        onclick="InjectionWarehouseModule.openConfirmStockBaselineModal('${_emJs}','${_epJs}','${_ecJs}')">
                        <span class="material-symbols-outlined" style="font-size:0.9rem;">flag</span> 현재고 확정
                    </button>` : ''}
    ...
    /** 이 날짜로 저장하면 이미 있는 "현재고 확정"(기준점)보다 이전이라 재고 계산에서
     *  조용히 제외되는지 확인한다 — _recordsForStockCalc가 기준점 이전 이동을 전부 걸러내기
     *  때문에, 관리자가 과거 날짜를 보정해도 "저장 성공"이라고만 뜨고 실제 재고 숫자는
     *  전혀 안 바뀌어 혼란을 준다(실제로 이런 사고가 있었다 — painting.js의 "사용일 변경"
     *  건도 같은 원리로 리스크가 있으나, 도장현장 재고는 기준재고 확정 기능이 없어 해당 없음). */
    function _baselineWarningFor(carModel, partName, color, newDate) {
        if (!newDate) return '';
        const all = _filterProductRecords(carModel, partName, color);
        const baseline = _findLatestStockBaseline(all);
        if (!baseline) return '';
        const cutDay = InvCalc.recordStamp(baseline).slice(0, 10);
        const editDay = String(newDate).slice(0, 10);
        if (!cutDay || !editDay || editDay >= cutDay) return '';
        return `⚠ 이 품목은 ${cutDay}에 "현재고 확정"(기준점)이 이미 있습니다.\n` +
            `기준점보다 이전 날짜(${editDay})로 저장하면, 이 보정은 재고 계산에서 조용히 제외되어 ` +
            `화면 재고 수치에는 반영되지 않습니다(과거 이력에는 남습니다).\n\n` +
            `그래도 이 날짜로 저장하시겠습니까?`;
    }
    ...
    async function saveLotEdit(carModel, partName, color, oldLot, oldQty) {
        ...
        const proceed = () => {
            ...
            if (delta !== 0) {
                ...
                const currentTotal = InvCalc.lotBalances(_recordsForStockCalc(productItems)).total;
        ...
        const baselineWarning = _baselineWarningFor(carModel, partName, color, newDate);
        if (baselineWarning) {
            UIUtils.confirm(baselineWarning, afterBaselineCheck);
            return;
        }
        afterBaselineCheck();
    }
        ...
        body.innerHTML = list.map(function (r) {
            ...
            return `<tr>
                ...
                <td style="white-space:nowrap;">
                    ...
                    ${isAdmin ? `<button type="button" class="btn btn-sm btn-danger" style="padding:4px 8px;font-size:0.78rem;margin-left:4px;"
                        title="이 반납 건을 취소(삭제)합니다 — 관리자 전용" onclick="InjectionWarehouseModule.cancelSiteReturn('${r.id}')">
    ...
    function openConfirmSiteReturnModal(id) {
        ...
        UIUtils.showModal('도장현장 반납 입고 처리', `
            ...
            <div class="form-group" style="margin-bottom:12px;">
                ...
                ${colorMissing ? '<div style="font-size:0.74rem;color:var(--accent-red);margin-top:3px;">이 반납 건에는 컬러 정보가 없습니다. 실물을 보고 정확한 컬러를 입력해야 같은 품목으로 입고됩니다.</div>' : ''}
            </div>
    ...
    /** 관리자 — 아직 입고 처리 전(반납 대기)인 반납 건을 취소한다.
     *  잘못된 사유로 등록됐거나, 관련 없는 LOT까지 한꺼번에 묶여 반납된 경우 되돌리는 용도. */
    async function cancelSiteReturn(id) {
        const isAdmin = typeof AuthModule !== 'undefined' && typeof AuthModule.isAdminUser === 'function' && AuthModule.isAdminUser();
        if (!isAdmin) {
            UIUtils.toast('관리자만 반납 건을 취소할 수 있습니다.', 'warning');
            return;
        }
        if (typeof PaintingInputModule === 'undefined' || !PaintingInputModule.cancelSiteReturn) {
            UIUtils.toast('반납 취소 기능을 불러올 수 없습니다.', 'error');
            return;
        }
        const list = PaintingInputModule.listPendingReturns();
        const r = list.find(function (x) { return String(x.id) === String(id); });
        ...
        const lotsTxt = (Array.isArray(r.lots) && r.lots.length)
            ...
            : (r.lotNo || '-');
        if (!confirm(
            '다음 반납 건을 취소(삭제)하시겠습니까?\n\n' +
            r.carModel + ' / ' + r.partName + (r.color ? ' / ' + r.color : '') + '\n' +
            'LOT: ' + lotsTxt + '\n합계: ' + UIUtils.formatNumber(r.quantity) + ' EA\n사유: ' + (r.returnReason || '-') +
            '\n\n삭제 후 복구할 수 없습니다. 도장현장에서 실제로 반납할 자재라면 다시 반납 처리해야 합니다.'
        )) return;
        try {
            await PaintingInputModule.cancelSiteReturn(id);
            UIUtils.toast('반납 건을 취소했습니다.', 'success');
            renderSiteReturns();
        } catch (e) {
            console.error('[InjectionWarehouseModule] 반납 취소 실패:', e);
            UIUtils.toast('반납 취소 실패: ' + (e && e.message ? e.message : e), 'error');
        }
    }
    ...
    function getLinkedInventoryForInspection(insp) {
        ...
        sourceLots.forEach(lot => {
            ...
            inventory.forEach(inv => {
                ...
                if (_isSiteReturnInbound(inv)) return;
    ...
    function findDuplicateLotInbounds() {
        const inventory = (Storage.getAll(STORE) || []).filter(function(d) {
            return d && d.type === '입고'
                && !_isStockErrorResetRecord(d)
                && !_isUnmatchedActionRecord(d)
                && !_isStockBaselineRecord(d)
                && !_isSiteReturnInbound(d)
                && !_isStockCountAdjustmentRecord(d);
        });
        ...
        UIUtils.showModal('이 LOT은 수입검사 입고 대기 항목입니다',
            `<div style="padding:4px 0;">
                ...
                <div style="margin-top:10px;padding:9px 12px;border-radius:8px;border:1px solid rgba(180,83,9,.35);
                    ...
                    <strong style="color:#b45309;">입력 수량 ${UIUtils.formatNumber(manualQty)} EA는 사용하지 않습니다.</strong>
                    수입검사 입고는 합격수량 <strong>${UIUtils.formatNumber(inspQty)} EA</strong>로만 등록됩니다.
                </div>` : ''}
    ...
    async function _commitManualInbound(linked) {
        ...
        if (linked && ctx.matches.length === 1) {
            ...
            const pend = ctx.matches[0].lots || [];
            const forcedLots = pend.map(function(l) {
                return { lotNo: String(l.lotNo || '').trim(), qty: _lotNum(l.qty) };
            }).filter(function(l) { return l.lotNo && l.qty > 0; });
            if (!forcedLots.length) {
                UIUtils.toast('검사 합격수량을 찾을 수 없습니다.', 'error');
                return;
            }
            data.lots = forcedLots;
            data.lotNo = forcedLots[0].lotNo;
            data.quantity = forcedLots.reduce(function(s, l) { return s + l.qty; }, 0);
            data.inspId = insp.id;
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    row.innerHTML = '<input type="text" class="form-input inv-lot-no" value="' + _escapeHtml(fillLotNo) + '" maxlength="6" readonly tabindex="-1" style="font-family:monospace; letter-spacing:1px; font-weight:800; font-size:1rem; background:var(--bg-secondary); cursor:not-allowed;">'
                        + '<input type="number" class="form-input inv-lot-qty" value="' + fillQty + '" readonly tabindex="-1" style="text-align:right; font-weight:800; font-size:1rem; color:var(--accent-blue); background:var(--bg-secondary); cursor:not-allowed;">'
                        + '<span></span>';
                    container.appendChild(row);
                    InjectionWarehouseModule.calcInvLotTotal();
                    _applyInspQtyLockToAddForm();
                }, 80);
            }, 80);
        }, 80);
    }

    function _applyInspQtyLockToAddForm() {
        if (!_lockedInspInbound) return;
        var wrap = document.getElementById('invLotRows');
        if (!wrap) return;
        wrap.querySelectorAll('.inv-lot-qty, .inv-lot-no').forEach(function(el) {
            el.readOnly = true;
            el.tabIndex = -1;
            el.style.background = 'var(--bg-secondary)';
            el.style.cursor = 'not-allowed';
        });
        wrap.querySelectorAll('.inv-lot-row button').forEach(function(b) { b.style.display = 'none'; });
        var addBtn = wrap.parentElement && wrap.parentElement.querySelector('button[onclick*="addInvLotRow"]');
        if (addBtn) addBtn.style.display = 'none';
        if (!document.getElementById('inspQtyLockNote')) {
            var note = document.createElement('div');
            note.id = 'inspQtyLockNote';
            note.style.cssText = 'margin:0 0 8px;padding:8px 12px;border-radius:8px;border:1px solid rgba(37,99,235,.3);background:rgba(37,99,235,.06);font-size:0.82rem;line-height:1.5;color:var(--text-secondary);';
            note.innerHTML = '<strong style="color:var(--accent-blue);">수입검사 합격수량</strong> — 수량은 변경할 수 없습니다.';
    ...
    function openAddModal(type = '입고') {
        ...
        // 첫 LOT 행 초기화
        setTimeout(() => {
            if (!_lockedInspInbound) addInvLotRow();
    ...
    function addInvLotRow() {
        if (_lockedInspInbound) {
            UIUtils.toast('수입검사 입고는 LOT·수량을 추가할 수 없습니다.', 'warning');
            return;
        }
        const container = document.getElementById('invLotRows');
        if (!container) return;
    ...
    function removeInvLotRow(btn) {
        if (_lockedInspInbound) {
            UIUtils.toast('수입검사 입고는 LOT·수량을 변경할 수 없습니다.', 'warning');
            return;
        }
    ...
    function _getLotBalancesForProduct(carModel, partName, color) {
        const allRecords = _filterProductRecords(carModel, partName, color);
        const records = _recordsForStockCalc(allRecords);
        ...
        return { balance: result.balance, lots: result.lots, records: records, allRecords: allRecords };
    }
    ...
    function _injectAliasOrphanTiles(mergedMap, materials, data) {
        ...
        inv.forEach(function(d) {
            ...
            const bal = InvCalc.lotBalances(_recordsForStockCalc(records));
            mergedMap[key] = {
                carModel: carModel,
                partName: partName,
                color: color,
    ...
    // ── 재고실사 (품목·LOT별 장부수량 vs 실사수량 일괄 대조 → 보정 + 현재고 확정) ──
    // "수량 보정"(LOT별)과 "현재고 확정"을 한 화면·한 흐름으로 묶은 도구.
    // 원본 입·출고 기록은 건드리지 않고 차이만큼 보정 레코드를 추가한 뒤,
    // 그 품목만 현재고 확정(기준점)까지 이어서 처리한다 — 두 안전장치(중복보정 감지는
    // 사유 필수 + 저장 전 검토 요약으로 대체) 원칙은 개별 LOT 보정 도구와 동일하다.
    /** 차종 정렬 순위 — 양산품(또는 구분 미지정) 차종이 먼저, A/S품만 있는 차종은 뒤로 */
    function _carModelSortRank(carModel) {
        const products = Storage.getAll(DB.STORES.PRODUCTS) || [];
        const materials = Storage.getAll(DB.STORES.INJECTION_MATERIALS) || [];
        const types = new Set();
        products.filter(function (p) { return p.carModel === carModel; }).forEach(function (p) { if (p.itemType) types.add(p.itemType); });
        materials.filter(function (m) { return m.carModel === carModel; }).forEach(function (m) { if (m.itemType) types.add(m.itemType); });
        if (!types.size) return 0; // 구분 정보 없음 → 양산 그룹 취급
        // PRODUCTS.itemType은 'A/S품', INJECTION_MATERIALS.itemType은 'A/S'로 표기가 달라 정규화 후 비교한다
        const hasNonAs = [...types].some(function (t) { return _shortItemType(t) !== 'A/S'; });
        return hasNonAs ? 0 : 1; // 0=양산(또는 혼합), 1=A/S품 전용
    }

    function openStockCountModal() {
        if (!_canEditWarehouseLot()) {
            UIUtils.toast('수량 보정 권한이 있는 사용자만 재고실사를 진행할 수 있습니다.', 'warning');
            return;
        }
        window._injCountAllRows = _buildStockCountRows();
        window._injCountRows = [];
        window._injCountCurrentCarModel = '';
        const carModels = [...new Set((window._injCountAllRows || []).map(function (g) { return g.carModel; }).filter(Boolean))]
            .sort(function (a, b) {
                return (_carModelSortRank(a) - _carModelSortRank(b)) || a.localeCompare(b, 'ko');
            });

        const _isAdminNow = _isAdminUser();
        UIUtils.showModal('사출 창고 재고실사', `
            <div style="background:var(--bg-secondary);border-radius:8px;padding:12px 14px;margin-bottom:12px;font-size:0.84rem;line-height:1.6;">
                <div style="font-weight:700;margin-bottom:4px;">차종을 먼저 선택한 뒤, 그 차종의 품목·LOT별로 실사한 실제 수량을 입력하세요.</div>
                <div style="color:var(--text-secondary);">
                    값을 바꾸지 않은 행은 그대로 유지됩니다(변경 없음). 저장하면 <strong>차이만큼 보정 기록</strong>이 남습니다.
                    ${_isAdminNow
                        ? `관리자 계정이라 저장 시 <strong>현재고 확정(기준점)</strong>까지 함께 처리됩니다.`
                        : `<strong>현재고 확정(기준점)은 관리자만</strong> 처리합니다 — 지금 저장하면 보정 기록만 남고, 관리자가 품목 상세에서 확인 후 별도로 확정합니다.`}
                    과거 입고·출고 이력은 삭제되지 않고 조회용으로 남습니다.
                </div>
            </div>
            <div style="display:flex;gap:10px;align-items:center;margin-bottom:10px;flex-wrap:wrap;">
                <select class="form-select" id="injCountCarModel" style="max-width:180px;" onchange="InjectionWarehouseModule.onStockCountCarModelChange()">
                    <option value="">차종 선택</option>
                    ${carModels.map(function (c) { return `<option value="${_escapeHtml(c)}">${_escapeHtml(c)}</option>`; }).join('')}
                </select>
                <input type="text" class="form-input" id="injCountSearch" placeholder="품명/컬러/LOT 검색" style="max-width:220px;" oninput="InjectionWarehouseModule.filterStockCountRows()">
                <span id="injCountSummary" style="font-size:0.82rem;color:var(--text-muted);margin-left:auto;">변경사항 없음</span>
            </div>
            <div id="injCountLastDateBar" style="display:none;font-size:0.8rem;color:var(--text-secondary);background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:6px;padding:5px 10px;margin-bottom:8px;">
                <span class="material-symbols-outlined" style="font-size:0.95rem;vertical-align:-3px;margin-right:3px;">event</span><span id="injCountLastDateText"></span>
            </div>
            <div class="data-table-wrapper" style="max-height:420px;overflow:auto;border:1px solid var(--border-color);border-radius:8px;">
                <table class="data-table compact" style="width:100%;font-size:0.82rem;">
                    <thead style="position:sticky;top:0;background:var(--bg-primary);z-index:1;">
                        <tr>
                            <th>차종</th><th>품명</th><th>컬러</th><th>사출 LOT</th>
                            <th style="text-align:right;">장부수량</th>
                            <th style="text-align:right;min-width:100px;">실사수량</th>
                            <th style="text-align:right;">차이</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody id="injCountBody"></tbody>
                </table>
            </div>
            <div class="form-group" style="margin-top:12px;">
                <label class="form-label">실사 사유/메모 <span style="color:var(--accent-red)">*</span></label>
                <textarea id="injCountReason" class="form-textarea" rows="2" placeholder="예: 2026년 8월 정기 재고실사"></textarea>
            </div>
            ${_buildAdjustNotifyHtml('injCount')}
        `, `
            <button class="btn btn-secondary" onclick="UIUtils.closeModal()">취소</button>
            <button class="btn btn-primary" style="background:#0f766e;border-color:#0f766e;" onclick="InjectionWarehouseModule.reviewStockCount()">
                <span class="material-symbols-outlined">fact_check</span> 검토 후 저장
            </button>
        `, 'xl');

        setTimeout(_renderStockCountBody, 30);
    }

    /** 차종 드롭다운 변경 — 입력 중인 미저장 값이 있으면 확인 후 전환 */
    function onStockCountCarModelChange() {
        const sel = document.getElementById('injCountCarModel');
        const newCarModel = sel ? sel.value : '';
        const prevCarModel = window._injCountCurrentCarModel || '';
        if (_hasUnsavedCountChanges()) {
            UIUtils.confirm(
                '입력 중인 실사수량이 아직 저장되지 않았습니다. 차종을 바꾸면 지금까지 입력한 내용이 사라집니다.\n계속할까요?',
                function () { _applyStockCountCarModel(newCarModel); },
                function () { if (sel) sel.value = prevCarModel; }
            );
            return;
        }
        _applyStockCountCarModel(newCarModel);
    }

    function _hasUnsavedCountChanges() {
        let changed = false;
        document.querySelectorAll('#injCountBody tr[data-row]').forEach(function (tr) {
            const book = Number(tr.dataset.book) || 0;
            const qtyInput = tr.querySelector('.inj-count-qty');
            const qty = Math.max(0, Math.round(Number(qtyInput ? qtyInput.value : 0) || 0));
            const isNew = tr.dataset.new === '1';
            if (isNew && qty <= 0) return;
            if (qty !== book) changed = true;
        });
        return changed;
    }

    function _applyStockCountCarModel(carModel) {
        window._injCountCurrentCarModel = carModel;
        window._injCountRows = carModel
            ? (window._injCountAllRows || []).filter(function (g) { return g.carModel === carModel; })
            : [];
        const searchEl = document.getElementById('injCountSearch');
        if (searchEl) searchEl.value = '';
        _renderCountLastDateBar(carModel);
        _renderStockCountBody();
    }

    /** 선택한 차종의 직전 재고실사일 — 이전 세션의 보정/현재고 확정 기록 중 가장 최근 날짜 */
    function _lastStockCountDate(carModel) {
        if (!carModel) return '';
        const rows = Storage.getAll(STORE) || [];
        let latest = '';
        rows.forEach(function (r) {
            if (r.carModel !== carModel) return;
            if (!r.isStockCountAdjustment && !r.isStockBaseline) return;
    ...
    function _renderCountLastDateBar(carModel) {
        const bar = document.getElementById('injCountLastDateBar');
        const text = document.getElementById('injCountLastDateText');
        if (!bar || !text) return;
        if (!carModel) { bar.style.display = 'none'; return; }
        const lastDate = _lastStockCountDate(carModel);
        text.textContent = lastDate ? `직전 실사일: ${lastDate}` : '직전 실사 이력 없음';
        bar.style.display = 'block';
    }

    /** 재고실사 대상 행 구성 — 사출자재 마스터·원장에 등장하는 모든 품목(재고 0 포함)을 그룹핑 */
    function _buildStockCountRows() {
        const stockMap = _calcStockMap();
        return Object.values(stockMap)
            .filter(function (g) { return g.partName; })
            .sort(function (a, b) {
    ...
    const _INJ_COUNT_TD = 'padding:2px 6px;line-height:1.25;';

    function _renderStockCountBody() {
        const body = document.getElementById('injCountBody');
        if (!body) return;
        const groups = window._injCountRows || [];

        if (!window._injCountCurrentCarModel) {
            body.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--text-muted);">차종을 선택하면 해당 차종의 품목이 표시됩니다.</td></tr>`;
            _updateCountSummary();
            return;
        }
        ...
        groups.forEach(function (g) {
            const { lots } = _getLotBalancesForProduct(g.carModel, g.partName, g.color);
            const validLots = (lots || []).filter(function (l) {
                return l.lotNo !== InvCalc.UNMATCHED && (Number(l.qty) || 0) > 0;
            });
            const groupAttrs = `data-gcar="${_escapeHtml(g.carModel)}" data-gpart="${_escapeHtml(g.partName)}" data-gcolor="${_escapeHtml(g.color || '')}"`;
            const groupSearch = _escapeHtml((g.carModel + ' ' + g.partName + ' ' + (g.color || '')).toLowerCase());

            if (validLots.length) {
                validLots.forEach(function (l) {
                    html += _countRowHtml(g, { lotNo: l.lotNo, bookQty: Number(l.qty) || 0, date: l.date }, false);
                });
                if (validLots.length > 1) {
                    const bookTotal = validLots.reduce(function (s, l) { return s + (Number(l.qty) || 0); }, 0);
                    html += _subtotalRowHtml(g, bookTotal);
                }
            } else {
                html += `<tr class="inj-count-empty-row" ${groupAttrs} data-search="${groupSearch}">
                    <td style="${_INJ_COUNT_TD}">${_escapeHtml(g.carModel)}</td><td style="${_INJ_COUNT_TD}">${_escapeHtml(g.partName)}</td><td style="${_INJ_COUNT_TD}">${_escapeHtml(g.color || '-')}</td>
                    <td colspan="4" style="${_INJ_COUNT_TD}color:var(--text-muted);">재고 0 · 실사에서 발견되면 우측 "LOT 추가"로 입력</td>
                    <td style="${_INJ_COUNT_TD}"></td>
                </tr>`;
            }
            html += `<tr class="inj-count-addrow" ${groupAttrs} data-search="${groupSearch}" style="background:var(--bg-primary);border-bottom:2px solid var(--border-color);">
                <td colspan="8" style="padding:3px 6px;">
                    <button type="button" class="btn btn-sm btn-outline" style="display:inline-flex;align-items:center;gap:3px;font-size:0.72rem;padding:2px 8px;" onclick="InjectionWarehouseModule._addCountLotRow(this)">
                        <span class="material-symbols-outlined" style="font-size:0.9rem;">add</span> LOT 추가
                    </button>
                </td>
            </tr>`;
        });

        body.innerHTML = html || `<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--text-muted);">대상 품목이 없습니다.</td></tr>`;
        _updateCountSummary();
    }

    function _countRowHtml(g, l, isNewRow) {
        const car = _escapeHtml(g.carModel), part = _escapeHtml(g.partName), color = _escapeHtml(g.color || '');
        const lotNo = l.lotNo || '';
        const searchKey = _escapeHtml((g.carModel + ' ' + g.partName + ' ' + (g.color || '') + ' ' + lotNo).toLowerCase());
        const bookQty = isNewRow ? 0 : (Number(l.bookQty) || 0);
        const lotCell = isNewRow
            ? `<input type="text" class="form-input inj-count-lot" maxlength="6" placeholder="YYMMDD" style="width:82px;padding:2px 6px;height:24px;font-family:monospace;">`
            : `${_escapeHtml(lotNo)}${l.date ? `<span style="font-size:0.68rem;color:var(--text-muted);margin-left:5px;">${_escapeHtml(String(l.date).slice(0, 10))}</span>` : ''}`;
        return `<tr data-row="1" data-car="${car}" data-part="${part}" data-color="${color}" data-lot="${_escapeHtml(lotNo)}"
                data-book="${bookQty}" data-new="${isNewRow ? 1 : 0}" data-search="${searchKey}">
            <td style="${_INJ_COUNT_TD}">${car}</td><td style="${_INJ_COUNT_TD}">${part}</td><td style="${_INJ_COUNT_TD}">${color || '-'}</td>
            <td style="${_INJ_COUNT_TD}">${lotCell}</td>
            <td style="${_INJ_COUNT_TD}text-align:right;">${isNewRow ? '0' : UIUtils.formatNumber(bookQty)}</td>
            <td style="${_INJ_COUNT_TD}text-align:right;">
                <input type="number" min="0" class="form-input inj-count-qty" value="${isNewRow ? '' : bookQty}" placeholder="0"
                    style="width:84px;height:24px;padding:2px 6px;text-align:right;" oninput="InjectionWarehouseModule._onCountQtyInput(this)">
            </td>
            <td style="${_INJ_COUNT_TD}text-align:right;" class="inj-count-diff">${isNewRow ? '' : '0'}</td>
            <td style="${_INJ_COUNT_TD}"></td>
        </tr>`;
    }

    /** 품목의 LOT이 2개 이상일 때 붙는 장부/실사 합계 행 — 실사수량 합계·차이 합계는 실시간 갱신 */
    function _subtotalRowHtml(g, bookTotal) {
        const car = _escapeHtml(g.carModel), part = _escapeHtml(g.partName), color = _escapeHtml(g.color || '');
        const groupSearch = _escapeHtml((g.carModel + ' ' + g.partName + ' ' + (g.color || '')).toLowerCase());
        return `<tr class="inj-count-subtotal" data-car="${car}" data-part="${part}" data-color="${color}" data-search="${groupSearch}"
                style="background:var(--bg-tertiary);border-top:1px solid var(--border-color);">
            <td colspan="4" style="${_INJ_COUNT_TD}text-align:right;color:var(--text-muted);">합계</td>
            <td style="${_INJ_COUNT_TD}text-align:right;font-weight:700;">${UIUtils.formatNumber(bookTotal)}</td>
            <td style="${_INJ_COUNT_TD}text-align:right;font-weight:700;" class="inj-count-subtotal-qty">${UIUtils.formatNumber(bookTotal)}</td>
            <td style="${_INJ_COUNT_TD}text-align:right;font-weight:700;" class="inj-count-subtotal-diff">0</td>
            <td style="${_INJ_COUNT_TD}"></td>
        </tr>`;
    }

    function _addCountLotRow(btn) {
        const tr = btn.closest('tr');
        if (!tr) return;
        const g = { carModel: tr.dataset.gcar || '', partName: tr.dataset.gpart || '', color: tr.dataset.gcolor || '' };

        // 재고 0(안내) 행이 아직 남아있으면 새 행을 추가하지 않고 그 자리를 바로 입력 행으로 전환
        // — 차종/품명/컬러가 두 줄에 중복 표시되는 것을 막는다
        const body = document.getElementById('injCountBody');
        let emptyRow = null;
        if (body) {
            body.querySelectorAll('tr.inj-count-empty-row').forEach(function (r) {
                if (r.dataset.gcar === g.carModel && r.dataset.gpart === g.partName && r.dataset.gcolor === (g.color || '')) {
                    emptyRow = r;
                }
            });
        }
        if (emptyRow) {
            emptyRow.outerHTML = _countRowHtml(g, { lotNo: '', bookQty: 0 }, true);
        } else {
            tr.insertAdjacentHTML('beforebegin', _countRowHtml(g, { lotNo: '', bookQty: 0 }, true));
        }

        // LOT이 2개 이상이 됐는데 합계 행이 아직 없으면 새로 추가
        if (body) {
            let count = 0, bookSum = 0, hasSubtotal = false;
            body.querySelectorAll('tr').forEach(function (r) {
                const sameGroup = r.dataset.car === g.carModel && r.dataset.part === g.partName && r.dataset.color === (g.color || '');
                if (!sameGroup) return;
                if (r.classList.contains('inj-count-subtotal')) { hasSubtotal = true; return; }
                if (r.dataset.row) { count++; bookSum += Number(r.dataset.book) || 0; }
            });
            if (count >= 2 && !hasSubtotal) {
                tr.insertAdjacentHTML('beforebegin', _subtotalRowHtml(g, bookSum));
            }
        }
        _applyCountSearchFilter();
        _updateGroupSubtotal(g.carModel, g.partName, g.color || '');
    }

    function filterStockCountRows() {
        _applyCountSearchFilter();
    }

    function _applyCountSearchFilter() {
        const q = ((document.getElementById('injCountSearch') || {}).value || '').trim().toLowerCase();
        document.querySelectorAll('#injCountBody tr').forEach(function (tr) {
            const key = tr.dataset.search || '';
            tr.style.display = (!q || key.indexOf(q) >= 0) ? '' : 'none';
        });
    }

    /** LOT 여러 개인 품목의 합계 행(장부/실사/차이 합계)을 실시간으로 갱신 */
    function _updateGroupSubtotal(car, part, color) {
        const body = document.getElementById('injCountBody');
        if (!body) return;
        let subtotalRow = null, bookSum = 0, qtySum = 0;
        body.querySelectorAll('tr').forEach(function (r) {
            const sameGroup = r.dataset.car === car && r.dataset.part === part && r.dataset.color === color;
            if (!sameGroup) return;
            if (r.classList.contains('inj-count-subtotal')) { subtotalRow = r; return; }
            if (r.dataset.row) {
                bookSum += Number(r.dataset.book) || 0;
                const qi = r.querySelector('.inj-count-qty');
                qtySum += Math.max(0, Math.round(Number(qi ? qi.value : 0) || 0));
            }
        });
        if (!subtotalRow) return;
        const diffSum = qtySum - bookSum;
        const qtyCell = subtotalRow.querySelector('.inj-count-subtotal-qty');
        const diffCell = subtotalRow.querySelector('.inj-count-subtotal-diff');
        if (qtyCell) qtyCell.textContent = UIUtils.formatNumber(qtySum);
        if (diffCell) {
            diffCell.textContent = diffSum === 0 ? '0' : ((diffSum > 0 ? '+' : '') + UIUtils.formatNumber(diffSum));
            diffCell.style.color = diffSum > 0 ? 'var(--accent-green)' : (diffSum < 0 ? 'var(--accent-red)' : 'var(--text-muted)');
        }
    }

    function _onCountQtyInput(el) {
        const tr = el.closest('tr');
        if (!tr) return;
        const book = Number(tr.dataset.book) || 0;
        const val = Math.max(0, Math.round(Number(el.value) || 0));
        const diff = val - book;
        const diffCell = tr.querySelector('.inj-count-diff');
        if (diffCell) {
            diffCell.textContent = diff === 0 ? '0' : ((diff > 0 ? '+' : '') + UIUtils.formatNumber(diff));
            diffCell.style.color = diff > 0 ? 'var(--accent-green)' : (diff < 0 ? 'var(--accent-red)' : 'var(--text-muted)');
            diffCell.style.fontWeight = diff !== 0 ? '700' : '400';
        }
        _updateGroupSubtotal(tr.dataset.car, tr.dataset.part, tr.dataset.color);
        _updateCountSummary();
    }

    function _updateCountSummary() {
        const el = document.getElementById('injCountSummary');
        if (!el) return;
        let lotCount = 0, totalDiff = 0;
        ...
        document.querySelectorAll('#injCountBody tr[data-row]').forEach(function (tr) {
            ...
            lotCount++;
            totalDiff += diff;
            productKeys.add(`${tr.dataset.car}||${tr.dataset.part}||${tr.dataset.color}`);
        });
        el.textContent = lotCount
            ? `변경 예정: ${productKeys.size}개 품목 · ${lotCount}개 LOT · 순변화 ${totalDiff > 0 ? '+' : ''}${UIUtils.formatNumber(totalDiff)} EA`
            : '변경사항 없음';
    }

    /** 검토 요약 확인 후 저장 — 실제 반영은 사용자가 요약을 확인하고 한 번 더 확정해야 진행된다 */
    function reviewStockCount() {
        if (!_canEditWarehouseLot()) {
            UIUtils.toast('수량 보정 권한이 있는 사용자만 재고실사를 저장할 수 있습니다.', 'warning');
            return;
        }
        const reasonEl = document.getElementById('injCountReason');
        const reason = reasonEl ? reasonEl.value.trim() : '';
        if (!reason) {
            UIUtils.toast('실사 사유/메모를 입력해주세요.', 'warning');
            if (reasonEl) reasonEl.focus();
            return;
        }
        ...
        body.querySelectorAll('tr[data-row]').forEach(function (tr) {
            const carModel = tr.dataset.car || '';
            const partName = tr.dataset.part || '';
            const color = tr.dataset.color || '';
            const isNew = tr.dataset.new === '1';
            const bookQty = Number(tr.dataset.book) || 0;
            const qtyInput = tr.querySelector('.inj-count-qty');
            const countQty = Math.max(0, Math.round(Number(qtyInput ? qtyInput.value : 0) || 0));
            let lotNo = tr.dataset.lot || '';

            if (isNew) {
                if (countQty <= 0) return; // 신규 행인데 수량 미입력 → 무시
                const lotInput = tr.querySelector('.inj-count-lot');
                lotNo = (lotInput ? lotInput.value : '').trim();
                const err = _yymmddLotError(lotNo);
                if (err) { lotErrors.push(`${partName}${color ? ' · ' + color : ''}: ${err}`); return; }
            }

            const diff = countQty - bookQty;
            if (diff === 0) return;
            changes.push({ carModel, partName, color, lotNo, bookQty, countQty, diff, isNew });
        });

        if (lotErrors.length) {
            UIUtils.toast('LOT번호를 확인하세요: ' + lotErrors.slice(0, 3).join(' / '), 'warning');
            return;
        }
        if (!changes.length) {
        ...
        const totalDiff = changes.reduce(function (s, c) { return s + c.diff; }, 0);
        const productKeys = new Set(changes.map(function (c) { return `${c.carModel}||${c.partName}||${c.color}`; }));
        const preview = changes.slice(0, 8).map(function (c) {
            return `${c.partName}${c.color ? (' · ' + c.color) : ''} · LOT ${c.lotNo}${c.isNew ? '(신규)' : ''} · `
                + `${UIUtils.formatNumber(c.bookQty)} → ${UIUtils.formatNumber(c.countQty)} (${c.diff > 0 ? '+' : ''}${UIUtils.formatNumber(c.diff)})`;
        }).join('\n');
        const more = changes.length > 8 ? `\n... 외 ${changes.length - 8}건` : '';

        const carModelLabel = changes[0] ? changes[0].carModel : '';
        const confirmTail = _isAdminUser()
            ? '저장하면 보정 기록을 남기고, 보정된 품목의 현재고를 새 기준점(현재고 확정)으로 저장합니다.'
            : '저장하면 보정 기록만 남습니다. 현재고 확정(기준점)은 관리자가 품목 상세에서 별도로 확인 후 진행합니다.';
        UIUtils.confirm(
            `[${carModelLabel}] ${productKeys.size}개 품목 · ${changes.length}개 LOT 보정 예정 (순변화 ${totalDiff > 0 ? '+' : ''}${UIUtils.formatNumber(totalDiff)} EA)\n\n${preview}${more}\n\n`
            + `${confirmTail}\n계속할까요?`,
            function () { _commitStockCount(changes, reason); }
        );
    }

    async function _commitStockCount(changes, reason) {
        const isAdmin = _isAdminUser();
        const notifyEnabled = !!(document.getElementById('injCountNotifyEnable') || {}).checked;
        const notifyUserIds = Array.from(document.querySelectorAll('.injCount-notify-user:checked'))
            .map(function (el) { return String(el.value || '').trim(); })
            .filter(Boolean);

        // 품목별로 묶어서 순차 처리 — 보정 기록은 담당자든 관리자든 항상 즉시 반영한다.
        // 현재고 확정(기준점)은 관리자만 — 담당자가 저장하면 보정만 남고, 관리자가 품목 상세의
        // "현재고 확정"으로 별도 확인 후 마무리한다(입력·확정 역할 분리, 실수로 잘못된 실사수량이
        // 바로 기준점으로 얼어붙는 것을 막기 위함).
        const byProduct = {};
        const order = [];
        changes.forEach(function (c) {
            const key = `${c.carModel}||${c.partName}||${c.color}`;
            if (!byProduct[key]) {
                byProduct[key] = { carModel: c.carModel, partName: c.partName, color: c.color, items: [] };
                order.push(key);
            }
            byProduct[key].items.push(c);
        });

        const actor = _getResetActorFields();
        const sessionId = 'SC' + Date.now();
        let okProducts = 0, failProducts = 0, lotCount = 0, confirmedProducts = 0;
        const failedNames = [];

        for (const key of order) {
            const g = byProduct[key];
            try {
                const material = _findInjectionMaterial(g.carModel, g.partName, g.color);
                for (const c of g.items) {
                    ...
                    await _addInventoryRecord({
                        ...
                        supplier: material ? (material.supplier || '') : '',
                        lotNo: c.lotNo,
                        lots: [{ lotNo: c.lotNo, qty: adjQty }],
                        quantity: adjQty,
                        unit: 'EA',
                        source: '재고실사 보정',
                        isStockCountAdjustment: true,
                        stockCountSession: sessionId,
                        resetReason: reason,
                        note: `[재고실사] ${reason} · ${UIUtils.formatNumber(c.bookQty)} → ${UIUtils.formatNumber(c.countQty)} EA`,
                        injMaterialId: material ? material.id : undefined,
                        ..._actorFieldsForRecord(c.diff > 0 ? '입고' : '출고')
                    });
                    lotCount++;
                }

                // 현재고 확정(기준점)은 관리자만 — confirmStockBaseline과 동일한 방식
                let baselineQty = null, baselineLotCount = null;
                if (isAdmin) {
                    const productItems = _filterProductRecords(g.carModel, g.partName, g.color);
                    const calcRecs = _recordsForStockCalc(productItems);
                    const bal = InvCalc.lotBalances(calcRecs);
                    const lots = (bal.lots || [])
                        .filter(function (l) { return l.lotNo !== InvCalc.UNMATCHED && (Number(l.qty) || 0) > 0; })
                        .map(function (l) {
                            return { lotNo: String(l.lotNo || '').trim() || '무표기', qty: Number(l.qty) || 0, date: l.date || '', supplier: l.supplier || '' };
                        });
                    ...
                    await _addInventoryRecord({
                        ...
                        unit: 'EA',
                        source: '현재고 확정',
                        baselineAction: 'stock_baseline',
                        isStockBaseline: true,
                        baselineReason: `재고실사: ${reason}`,
                        stockCountSession: sessionId,
                        note: `[현재고 확정] 재고실사(${reason}) · ${UIUtils.formatNumber(total)} EA · LOT ${lots.length}개`,
                        injMaterialId: material ? material.id : undefined,
                        stockBeforeFullHistory: fullTotal,
                        stockAfterTarget: total,
                        ...actor
                    });
                    baselineQty = total;
                    baselineLotCount = lots.length;
                    confirmedProducts++;
                }

                await Storage.add(DB.STORES.INSPECTION_DELETE_LOGS, {
                    id: Storage.generateId(),
                    type: 'injection_inventory_stock_count',
                    typeLabel: '사출 창고 재고실사 보정' + (isAdmin ? ' (현재고 확정 포함)' : ' (확정 대기)'),
                    deletedAt: actor.resetAt || new Date().toISOString(),
                    deletedBy: actor.resetBy || '',
                    reason: reason,
                    originalData: {
                        carModel: g.carModel, partName: g.partName, color: g.color || '',
                        items: g.items, baselineQty: baselineQty, lotCount: baselineLotCount,
                        sessionId: sessionId, confirmed: isAdmin
                    },
                    summary: `${g.carModel} / ${g.partName} ${g.color || ''} / 실사 보정 ${g.items.length}건`
                        + (isAdmin ? ` · 확정 ${UIUtils.formatNumber(baselineQty)} EA` : ' · 확정 대기(관리자 필요)')
                });

                okProducts++;
            } catch (e) {
                console.error('[재고실사] 보정 실패:', g, e);
                failProducts++;
                failedNames.push(`${g.partName}${g.color ? ' · ' + g.color : ''}`);
            }
        }

        if (notifyEnabled && notifyUserIds.length && typeof AuthModule !== 'undefined' && typeof AuthModule.sendInternalMessage === 'function') {
            try {
                notifyUserIds.forEach(function (userId) {
                    AuthModule.sendInternalMessage({
                        targetType: 'user',
                        targetId: userId,
                        title: '사출창고 재고실사 반영 알림',
                        body: `품목 ${okProducts}건 보정 완료${isAdmin ? ' (현재고 확정 포함)' : ' — 현재고 확정은 관리자 확인 필요'}`
                            + `${failProducts ? ` · 실패 ${failProducts}건` : ''}\n사유: ${reason}`,
                        category: 'injection-warehouse',
                        priority: 'high'
                    });
                });
        ...
        UIUtils.closeModal();
        if (failProducts > 0) {
            UIUtils.toast(`재고실사 일부 실패 — 완료 ${okProducts}건 / 실패 ${failProducts}건 (${failedNames.slice(0, 3).join(', ')})`, 'warning');
        } else if (isAdmin) {
            UIUtils.toast(`재고실사 완료 — ${confirmedProducts}개 품목 · ${lotCount}개 LOT 보정 및 현재고 확정`, 'success');
        } else {
            UIUtils.toast(`재고실사 보정 저장 완료 — ${okProducts}개 품목 · ${lotCount}개 LOT. 현재고 확정은 관리자가 품목 상세에서 별도로 진행해야 합니다.`, 'success');
        }
        loadData();
    }
    ...
    async function _applyStockErrorCorrection(carModel, partName, color, reason, targetQty, lotNo) {
        ...
        const productItems = _recordsForStockCalc(_filterProductRecords(carModel, partName, resolvedColor));
    ...
    function openConfirmStockBaselineModal(carModelEnc, partNameEnc, colorEnc) {
        if (!_isAdminUser()) {
            UIUtils.toast('관리자만 현재고를 확정할 수 있습니다.', 'warning');
            return;
        }
        ...
        const color = decodeURIComponent(colorEnc || '');
        const all = _filterProductRecords(carModel, partName, color);
        const calcRecs = _recordsForStockCalc(all);
        const bal = InvCalc.lotBalances(calcRecs);
        const lots = (bal.lots || [])
            .filter(function (l) { return l.lotNo !== InvCalc.UNMATCHED && (Number(l.qty) || 0) > 0; })
            .slice()
        ...
        const total = lots.reduce(function (s, l) { return s + (Number(l.qty) || 0); }, 0);
        const existing = _findLatestStockBaseline(all);
        const lotRows = lots.length
            ? lots.map(function (l) {
                return `<tr>
                    <td style="white-space:nowrap;">${_escapeHtml(String(l.date || '-').slice(0, 16))}</td>
                    <td style="font-family:monospace;">${_escapeHtml(l.lotNo)}</td>
                    <td style="text-align:right;font-weight:700;">${UIUtils.formatNumber(l.qty)}</td>
                </tr>`;
            }).join('')
            : `<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:12px;">잔량 LOT 없음 (재고 0 EA로 확정)</td></tr>`;

        UIUtils.showModal('현재고 확정', `
            <div style="background:rgba(15,118,110,0.06);border:1px solid rgba(15,118,110,0.25);border-radius:8px;padding:12px 14px;margin-bottom:14px;font-size:0.86rem;line-height:1.6;">
                ...
                <div style="margin-top:6px;">확정 재고: <strong style="color:#0f766e;">${UIUtils.formatNumber(total)} EA</strong>
                    · LOT ${lots.length}개</div>
                <div style="color:var(--text-secondary);margin-top:6px;">
                    지금 잔량 LOT을 재고 계산 <strong>기준점</strong>으로 저장합니다.
                    <strong>과거 입고·출고 이력은 삭제되지 않으며</strong> 입고이력/출고이력 탭에도 그대로 남습니다.
                    이후 재고·출고 가능 LOT는 이 기준점 이후 이동만으로 계산됩니다.
                </div>
                ${existing ? `<div style="margin-top:8px;color:#b45309;">이미 기준점이 있습니다(${_escapeHtml(InvCalc.recordStamp(existing) || String(existing.date || ''))}). 새로 확정하면 그 이후부터 다시 계산합니다.</div>` : ''}
            </div>
            <div style="max-height:220px;overflow:auto;margin-bottom:12px;border:1px solid var(--border-color);border-radius:8px;">
                <table class="data-table compact" style="width:100%;font-size:0.82rem;">
                    <thead><tr>
                        <th>입고일</th><th>LOT</th><th style="text-align:right;">잔량</th>
                    </tr></thead>
                    <tbody>${lotRows}</tbody>
                </table>
            </div>
            <div class="form-group">
                <label class="form-label">확정 사유 <span style="color:var(--accent-red)">*</span></label>
                <textarea id="injStockBaselineReason" class="form-textarea" rows="3"
                    placeholder="예: 실사 재고 대조 완료 · 과거 이력은 조회용으로만 유지"></textarea>
            </div>
        `, `
            <button class="btn btn-secondary" onclick="UIUtils.closeModal()">취소</button>
            <button class="btn btn-primary" style="background:#0f766e;border-color:#0f766e;"
                onclick="InjectionWarehouseModule.confirmStockBaseline('${carModelEnc}','${partNameEnc}','${colorEnc}')">
                확정 저장
            </button>
        `, 'md');

        setTimeout(function () {
            const el = document.getElementById('injStockBaselineReason');
            if (el) el.focus();
        }, 100);
    }

    async function confirmStockBaseline(carModelEnc, partNameEnc, colorEnc) {
        if (!_isAdminUser()) {
            UIUtils.toast('관리자만 현재고를 확정할 수 있습니다.', 'warning');
            return;
        }
        const reasonEl = document.getElementById('injStockBaselineReason');
        const reason = reasonEl ? reasonEl.value.trim() : '';
        if (!reason) {
            UIUtils.toast('확정 사유를 입력해주세요.', 'warning');
            if (reasonEl) reasonEl.focus();
            return;
        }
        ...
        const materials = Storage.getAll(DB.STORES.INJECTION_MATERIALS) || [];
        const resolvedColor = _resolveMasterColor(carModel, partName, color, materials);
        const all = _filterProductRecords(carModel, partName, resolvedColor || color);
        const calcRecs = _recordsForStockCalc(all);
        const bal = InvCalc.lotBalances(calcRecs);
        const lots = (bal.lots || [])
            .filter(function (l) { return l.lotNo !== InvCalc.UNMATCHED && (Number(l.qty) || 0) > 0; })
        ...
        const record = {
            ...
            note: `[현재고 확정] ${reason} · ${UIUtils.formatNumber(total)} EA · LOT ${lots.length}개`,
            injMaterialId: material ? material.id : undefined,
            stockBeforeFullHistory: fullTotal,
            stockAfterTarget: total,
            ...actor
        ...
        try {
            await _addInventoryRecord(record);
            await Storage.add(DB.STORES.INSPECTION_DELETE_LOGS, {
                id: Storage.generateId(),
                type: 'injection_inventory_baseline',
                typeLabel: '사출 창고 현재고 확정',
                deletedAt: actor.resetAt || new Date().toISOString(),
                deletedBy: actor.resetBy || '',
                reason: reason,
                originalData: {
                    ...
                    lots: lots,
                    fullHistoryTotal: fullTotal
                },
                summary: `${carModel} / ${partName} ${color || ''} / 확정 ${UIUtils.formatNumber(total)} EA`
            });
            UIUtils.closeModal();
            UIUtils.toast(`현재고 확정 완료 · ${UIUtils.formatNumber(total)} EA`, 'success');
            loadData();
            setTimeout(function () {
                showPartDetail(carModel, partName, color);
            }, 80);
    ...
    async function saveNew() {
        ...
        if (_lockedInspInbound && data.type === '입고') {
            const lock = _lockedInspInbound;
            const insp = Storage.getById(DB.STORES.INJECTION_INSPECTIONS, lock.inspId);
            let lotNo = String(lock.lotNo || '').trim();
            let qty = _lotNum(lock.qty);
            if (insp) {
                const lot = (insp.lots || []).find(function(l) {
                    return String(l.lotNo || '').trim() === lotNo;
                });
                if (lot) qty = _lotNum(lot.qty);
                else if (!lotNo) {
                    lotNo = String(insp.lotNo || '').trim();
                    qty = _lotNum(insp.passQty);
                }
                data.inspDate = insp.date || data.inspDate;
            }
            if (!lotNo || qty <= 0) {
                UIUtils.toast('수입검사 합격수량을 확인할 수 없습니다.', 'error');
                return;
            }
            data.inspId = lock.inspId;
            data.lots = [{ lotNo: lotNo, qty: qty }];
            data.lotNo = lotNo;
            data.quantity = qty;
            const note = (((document.getElementById('addInvSource') || {}).value || '').trim());
            data.source = '수입검사 합격수량 전체입고';
        ...
        if (data.type === '입고' && !data.inspId && !_isSiteReturnInbound(data)) {
            await _ensurePendingCutoverLoaded();
            await _ensureDismissedPendingLoaded();
    ...
    async function _finalizeInventorySave(data, opts) {
        opts = opts || {};
        if (data.type === '입고') {
            const baselineWarning = _baselineWarningFor(data.carModel, data.partName, data.color, String(data.date || '').slice(0, 10));
            if (baselineWarning) {
                const proceed = await new Promise(function (resolve) {
                    UIUtils.confirm(baselineWarning, function () { resolve(true); }, function () { resolve(false); });
                });
                if (!proceed) return;
            }
        }
    ...
    function openEditModal(id) {
        ...
        const qtyLocked = _isInspectionQtyLockedRecord(d);
        const qtyLockAttr = qtyLocked ? ' readonly tabindex="-1" style="text-align:right;font-weight:700;background:var(--bg-secondary);cursor:not-allowed;"' : ' style="text-align:right;font-weight:700;"';
        const qtyFieldHtml = hasMultiLot
                    ...
                    ${d.lots.map(function (l, i) {
                        return `<div style="display:grid;grid-template-columns:1.2fr 1fr;gap:8px;align-items:center;margin-bottom:5px;">
                            ...
                            <input type="number" class="form-input edit-inv-lot-qty" data-lot="${_escapeHtml(l.lotNo || '')}"
                                value="${Number(l.qty) || 0}" min="0"${qtyLocked ? qtyLockAttr : ' style="text-align:right;font-weight:700;" oninput="InjectionWarehouseModule._updateEditInvLotTotal()"'}>
                        </div>`;
                    }).join('')}
                </div>
            ...
            : `<div class="form-group">
                ...
                <input type="number" class="form-input" id="editInvQty" value="${d.quantity || 0}" min="1"
                    ${qtyLocked ? 'readonly tabindex="-1" style="font-size:1.1rem;font-weight:700;text-align:right;background:var(--bg-secondary);cursor:not-allowed;"' : 'style="font-size:1.1rem;font-weight:700;text-align:right;"'}>
               </div>`;
        UIUtils.showModal(
    ...
    async function saveEdit(id) {
        ...
        if (orig && _isInspectionQtyLockedRecord(orig)) {
            await Storage.update(STORE, id, updates);
            UIUtils.closeModal();
            UIUtils.toast('비고만 수정되었습니다. 수입검사 입고 수량은 변경되지 않습니다.', 'success');
            loadData();
            return;
        }
    ...
    function showStockModal() {
        ...
        Object.keys(groups).forEach(function(key) {
            ...
            const balance = InvCalc.lotBalances(_recordsForStockCalc(g.records));
    ...
    return {
        ...
        cancelReworkShortage,
        openReworkDispatchFromShortage,
        toggleSiteInboundShortageNotify,
        ackReworkUsageFromWork,
        isReworkSourcedPart: _isReworkSourcedPart,
        _goToPlan,
        editReservedPlan,
