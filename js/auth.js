...
const AuthModule = (function () {
    ...
    function _emptyIncomingInspNotify() {
        const o = {};
        INCOMING_INSP_NOTIFY_KEYS.forEach(function (k) { o[k] = []; });
        return o;
    }
    ...
    const ALL_PAGES = [
        ...
        { id:'injection-layout',          label:'보관창고 레이아웃',  group:'창고' },
        ...
        { id:'shipping-overview',         label:'출하검사 현황',      group:'출하/제품' },
        { id:'shipping-standby',          label:'출하검사(외관)',     group:'출하/제품' },
        { id:'shipping-inspection',       label:'출하검사 일지',      group:'출하/제품' },
        { id:'shipping-reliability',      label:'출하검사(신뢰성)',   group:'출하/제품' },
        { id:'shipping-periodic-reli',    label:'신뢰성정기검사',     group:'출하/제품' },
        { id:'shipping-certificate',      label:'출하성적서 발행',    group:'출하/제품' },
        { id:'shipping-standard',         label:'출하검사 기준서',    group:'출하/제품' },
        ...
        { id:'overtime-plan',             label:'연장근무계획',       group:'생산관리' },
        { id:'prod-quality',              label:'초중종물 관리',      group:'공정품질' },
        { id:'prod-spc',                  label:'SPC 관리',           group:'공정품질' },
        { id:'spc-color',                 label:'색차 SPC',           group:'공정품질' },
        { id:'spc-film',                  label:'도막두께 SPC',       group:'공정품질' },
        { id:'spc-gloss',                 label:'광택 SPC',           group:'공정품질' },
    ...
    const PAGE_GROUPS = [
        ...
        { key:'painting_work',    label:'\ub3c4\uc7a5\uc791\uc5c5',       pages:['painting-process','painting-work-a','painting-work-b','painting-inspection','painting-rework-wip'] },
        ...
        { key:'overtime_plan',    label:'\uc5f0\uc7a5\uadfc\ubb34\uacc4\ud68d', pages:['overtime-plan'] },
        { key:'quality_prod',     label:'\ucd08\uc911\uc885\ubb3c \uad00\ub9ac', pages:['prod-quality'] },
        { key:'quality_spc',      label:'SPC \uad00\ub9ac',            pages:['prod-spc','spc-color','spc-film','spc-gloss'] },
        { key:'quality_perf',     label:'\ud488\uc9c8 \uc2e4\uc801',    pages:['quality-performance','painting-quality-performance'] },
        { key:'quality_improve',  label:'\uac1c\uc120\ud65c\ub3d9',     pages:['improvement-activity'] },
        { key:'quality_limit',    label:'\ud55c\ub3c4 \uacac\ubcf8',    pages:['limit-samples'] },
        { key:'quality_cert',     label:'\uc790\uaca9\uc778\uc99d \uad00\ub9ac', pages:['certifications-mgmt','cert-standard','cert-eval','cert-ledger','cert-multiskill-eval','cert-multiskill-analysis','gauge-rr-variable','gauge-rr-attribute','cert-status','inspectors-mgmt','operators-mgmt'] },
    ...
    const MENU_PERMISSION_SCOPES = {
        ...
        'shipping-overview': ['shipping-overview','shipping-standby','shipping-inspection','shipping-reliability','shipping-periodic-reli','shipping-certificate','shipping-standard','shipping-std-photo'],
        'shipping-standby': ['shipping-overview','shipping-standby','shipping-inspection','shipping-reliability','shipping-periodic-reli','shipping-certificate','shipping-standard','shipping-std-photo'],
        'shipping-inspection': ['shipping-overview','shipping-standby','shipping-inspection','shipping-reliability','shipping-periodic-reli','shipping-certificate','shipping-standard','shipping-std-photo'],
        'shipping-reliability': ['shipping-overview','shipping-standby','shipping-inspection','shipping-reliability','shipping-periodic-reli','shipping-certificate','shipping-standard','shipping-std-photo'],
        'shipping-periodic-reli': ['shipping-overview','shipping-standby','shipping-inspection','shipping-reliability','shipping-periodic-reli','shipping-certificate','shipping-standard','shipping-std-photo'],
        ...
        'painting-process': ['painting-process','painting-work-a','painting-work-b','painting-inspection','painting-rework-wip'],
    ...
    function _defaultPerms() {
        ...
        return {
            ...
            quality_manager: rw([
                ...
                'incoming-overview','injection-incoming','paint-incoming-inspection',
                'warehouse-overview',
                'painting-inspection','painting-rework-wip','laser-inspection',
                'shipping-overview','shipping-standby','shipping-reliability','shipping-periodic-reli','product-warehouse',
                'prod-standards','prod-conditions','prod-quality','prod-spc','spc-color','spc-film','spc-gloss',
                'quality-performance','improvement-activity','limit-samples',
            ]),
            ...
            self_inspector: rw([
                'dashboard',
                'production-plan','overtime-plan',
                'painting-process','painting-work-a','painting-work-b','painting-inspection','painting-rework-wip',
        ...
        Object.keys(perms || {}).forEach(function(roleKey) {
            ...
            ['access', 'write'].forEach(function(field) {
                ...
                if (set.has('painting-work') || set.has('painting-work-a') || set.has('painting-work-b')
                    ...
                    || set.has('painting-input-a') || set.has('painting-input-b')) {
                    set.add('painting-process');
                    set.add('painting-work-a');
                    set.add('painting-work-b');
                    set.delete('painting-work');
                    set.delete('painting-input');
                    set.delete('painting-input-a');
                    set.delete('painting-input-b');
                }
    ...
    function isPageAccessGranted(roleKey, pageId) {
        ...
        if (pageId === 'painting-input' || pageId === 'painting-input-a' || pageId === 'painting-input-b') {
            return isPageAccessGranted(roleKey, 'painting-process');
        }
    ...
    function paintMixNotifyKindForLine(line) {
        const s = String(line || '').trim();
    ...
    function _snapshotIncomingInspNotify() {
        const src = _incomingInspNotify && typeof _incomingInspNotify === 'object' ? _incomingInspNotify : {};
        const out = _emptyIncomingInspNotify();
    ...
    function _normalizeIncomingInspNotify(raw) {
        if (Array.isArray(raw)) {
            ...
            const o = _emptyIncomingInspNotify();
            o.injection = ids.slice();
            o.paint = ids.slice();
            return o;
        }
        ...
        const o = _emptyIncomingInspNotify();
        INCOMING_INSP_NOTIFY_KEYS.forEach(function (k) {
            o[k] = _normalizeIncomingInspIdList(src[k]);
        });
        return o;
    }
    ...
    async function ensureIncomingInspNotifyRecipients(forceReload) {
        ...
        try {
            ...
            _incomingInspNotify = _emptyIncomingInspNotify();
        }
